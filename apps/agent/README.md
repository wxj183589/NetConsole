...
源码联调同样使用正式数据根；只有自动测试才可显式改用 `<repository-parent>\test-data\NetConsole\<run-id>`：
...
- 当前正式构建/验收目标为 Windows 10、Windows 11 和 Windows Server 2016 x64；Windows Server 2012/2012 R2 不属于后续开发、测试或发布范围；CentOS 7.4 Agent/离线包尚未实现；
- Agent 不做复杂业务分析；
