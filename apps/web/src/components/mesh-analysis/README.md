...
- 图表和 Tooltip 只消费解析事实层已接受的 `LinkCnt=1` 链路。`LinkCnt=0` 驱动占位行不进入 ACTIVE/STANDBY、RSSI、gap 或图表 payload；未知 `LinkCnt` 由解析器 diagnostic 后拒绝，不能在组件侧以 `RSSI=0` 作补救判断。
...
- RSSI `0` 在后端按自然秒分类：同一自然秒内的一条或多条 0 属于无有效 RSSI，连续覆盖多个自然秒为持续无有效 RSSI；此规则只适用于 `LinkCnt=1` 的有效链路。图表保留这些角色快照和状态元数据，但展示值一律为 `null`，不绘制或合成 `0` 端点。恢复时刻仅归属新有效样本，缺口采用 `[start, end)`，降采样仍固定保留恢复点和关键边界。主用链路信号只跳过前后均为唯一有效 ACTIVE、且没有真实缺口的单个 `MULTI_ACTIVE` 歧义帧；`NO_ACTIVE`、连续歧义和真实缺口仍断线。
