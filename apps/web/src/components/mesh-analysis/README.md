...
- RSSI `0` 在后端按自然秒分类：同一自然秒内的一条或多条 0 属于无有效 RSSI，连续覆盖多个自然秒为持续无有效 RSSI；图表保留这些角色快照和状态元数据，但展示值一律为 `null`，不绘制或合成 `0` 端点。恢复时刻仅归属新有效样本，缺口采用 `[start, end)`，降采样仍固定保留恢复点和关键边界。主用链路信号只跳过前后均为唯一有效 ACTIVE、且没有真实缺口的单个 `MULTI_ACTIVE` 歧义帧；`NO_ACTIVE`、连续歧义和真实缺口仍断线。
