...
- 图表的 data/display/theme/resize/viewport/reset 更新分开处理：显示 Peer、切换时刻线、切换节点、站点区间带、主题和容器尺寸变化都恢复真实毫秒视口，不把 `dataZoom` 重置到全日志。组件通过 `getViewport/applyViewport/resetViewport/getVisibleTimeRange` 暴露同一契约。
