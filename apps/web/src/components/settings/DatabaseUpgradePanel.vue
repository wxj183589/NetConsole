  ...
  <section class="database-upgrade-panel" v-loading="loading">
    <div class="panel-heading">
      ...
      <div class="panel-actions">
        <el-button v-if="isFeatureEnabled('web.database_legacy_archive_organize')" :icon="Upload" :loading="actionId === 'organize'" @click="organizeLegacy">{{ t('database_upgrade.organize_legacy', '整理历史归档') }}</el-button>
      ...
      <el-tab-pane :label="t('database_upgrade.status_tab', '数据库状态')">
        <div class="table-wrap">
          <el-table :data="databases" min-width="900" :empty-text="t('database_upgrade.no_databases', '当前局点没有已登记的数据库')">
            ...
            <el-table-column :label="t('database_upgrade.actions', '操作')" width="130" fixed="right"><template #default="{ row }"><el-button v-if="isFeatureEnabled('web.database_upgrade_start')" type="primary" size="small" :icon="Select" :disabled="!row.needs_upgrade" :loading="actionId === `upgrade:${row.mr_id}`" @click="upgrade(row.mr_id)">{{ t('database_upgrade.start', '立即升级') }}</el-button></template></el-table-column>
          </el-table>
        </div>
      </el-tab-pane>
            ...
            <el-table-column :label="t('database_upgrade.operation', '操作')" width="190" fixed="right">
              <template #default="{ row }">
                <div class="icon-actions">
                  <el-tooltip v-if="isFeatureEnabled('web.database_backup_validate')" :content="t('database_upgrade.validate', '验证备份')"><el-button :icon="Select" circle :loading="actionId === `validate:${row.backup_id}`" @click="validateBackup(row)" /></el-tooltip>
                  <el-tooltip v-if="isFeatureEnabled('web.database_backup_restore')" :content="t('database_upgrade.restore', '恢复此版本')"><el-button :icon="RefreshLeft" circle :disabled="row.integrity_check_result?.restorable !== true" :loading="actionId === `restore:${row.backup_id}`" @click="restoreBackup(row)" /></el-tooltip>
                  <el-tooltip v-if="isFeatureEnabled('web.database_backup_open_directory')" :content="t('database_upgrade.open_directory', '打开所在目录')"><el-button :icon="FolderOpened" circle :loading="actionId === `open:${row.backup_id}`" @click="openDirectory(row)" /></el-tooltip>
                  <el-tooltip v-if="isFeatureEnabled('web.database_backup_delete')" :content="t('database_upgrade.delete', '删除备份')"><el-button type="danger" plain :icon="Delete" circle :loading="actionId === `delete:${row.backup_id}`" @click="deleteBackup(row)" /></el-tooltip>
                </div>
              </template>
            </el-table-column>
          </el-table>
