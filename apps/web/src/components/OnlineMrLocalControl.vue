      ...
      <el-collapse v-if="!active" class="control-config">
        <el-collapse-item title="启动配置（凭据由设备库受控读取）" name="config">
          <el-form label-position="top">
            ...
            <el-divider content-position="left">fping</el-divider><el-switch v-model="config.fping.enabled" active-text="启用 fping" :disabled="realDeviceTest" />
            <div v-if="config.fping.enabled" class="form-grid"><el-form-item label="目标 IP"><el-input v-model="config.fping.target" /></el-form-item><el-form-item label="间隔 / 超时（ms）"><div class="inline-fields"><el-input-number v-model="config.fping.interval_ms" :min="10" :disabled="realDeviceTest" /><el-input-number v-model="config.fping.timeout_ms" :min="1" :disabled="realDeviceTest" /></div></el-form-item><el-form-item label="包大小"><el-input-number v-model="config.fping.packet_size" :min="1" :max="65535" /></el-form-item></div>
            ...
            <div v-if="config.iperf.enabled" class="form-grid"><el-form-item label="服务端 IP"><el-input v-model="config.iperf.server_ip" :disabled="realDeviceTest" /></el-form-item><el-form-item label="端口"><el-input-number v-model="config.iperf.port" :min="1" :max="65535" /></el-form-item><el-form-item label="协议"><el-select v-model="config.iperf.protocol" :disabled="realDeviceTest"><el-option label="TCP" value="TCP" /><el-option label="UDP" value="UDP" /></el-select></el-form-item><el-form-item label="并发流"><el-input-number v-model="config.iperf.parallel" :min="1" :max="32" :disabled="realDeviceTest" /></el-form-item><el-form-item v-if="config.iperf.protocol === 'UDP'" label="UDP 带宽 Mbps"><el-input-number v-model="config.iperf.udp_bitrate_mbps" :min="0.1" /></el-form-item><el-form-item v-else label="TCP 限速">{{ realDeviceTest ? '2 Mbps' : '未限制' }}</el-form-item><el-form-item label="方向"><el-switch v-model="config.iperf.reverse" active-text="Reverse" :disabled="realDeviceTest" /></el-form-item></div>
          </el-form>
        </el-collapse-item>
      </el-collapse>
