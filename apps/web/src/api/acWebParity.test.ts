...
import { exportAcExtensions, getAcWebTask, recoverAcWebTasks, startAcLocalRebuild } from './acWebParity'
...
describe('AC Web parity API client', () => {
  it('submits only the local rebuild target and exposes task recovery', async () => {
    ...
    expect(fetchMock.mock.calls[2][0]).toBe('/api/ac-management/web-tasks/task-1')
    expect(fetchMock.mock.calls[3][0]).toBe('/api/ac-management/web-tasks/recover')
  })
})
