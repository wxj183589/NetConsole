...
import { deleteAcFitAps, exportAcExtensions, getAcWebTask, importAcFitApMetadata, recoverAcWebTasks, startAcLocalRebuild, startAcResourceRefresh } from './acWebParity'
...
describe('AC Web parity API client', () => {
  it('submits only the local rebuild target and exposes task recovery', async () => {
    ...
    expect(fetchMock.mock.calls[4][0]).toBe('/api/ac-management/extensions/export?search=station-a&ac_id=ac-1')
    expect(fetchMock.mock.calls[4][1].method).toBe('POST')
    expect(fetchMock.mock.calls[5][0]).toBe('/api/ac-management/web-tasks/task-1')
    expect(fetchMock.mock.calls[6][0]).toBe('/api/ac-management/web-tasks/recover')
  })
})
