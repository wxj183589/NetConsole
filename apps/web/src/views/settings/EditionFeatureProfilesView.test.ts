...
vi.mock('../../features', () => ({ loadWebFeatures: vi.fn() }))
...
const items = (): FeatureSetting[] => [
  baseItem({}),
  baseItem({
    feature_id: 'web.rail_train_online',
    title: '列车在线情况',
  ...
  baseItem({
    ...
    parent_id: 'web.rail_train_online',
    item_type: 'action',
    configuration_layer: 'operation',
    dependencies: ['cap.train_online_data'],
    delivery_dependencies: ['cap.train_online_data'],
  }),
  baseItem({
    feature_id: 'cap.train_online_data',
    title: '列车在线数据能力',
    group_id: 'foundation',
    group_title: '基础与桌面',
...
const snapshot = (dependencyIssues = true): FeatureSettingsSnapshot => ({
  ...
  dependency_issues: dependencyIssues ? [{
    ...
    dependency_id: 'web.rail_train_online',
    dependency_title: '列车在线情况',
    issue_type: 'delivery_parent_missing',
    message: 'ac.mesh_link.refresh 的交付父级 web.rail_train_online 未纳入客户版',
...
describe('EditionFeatureProfilesView', () => {
  ...
  it('groups dependency issues and auto-fixes only the current draft', async () => {
    ...
    fixed.items = fixed.items.map((item) => item.feature_id === 'web.rail_train_online'
