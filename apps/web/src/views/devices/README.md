...
- 正式模板和导出在既有设备身份列后增加“建设阶段”“投运状态”“投运状态说明”；上一版 30 列、旧 28 列和 21 列文件仍可导入。新建设备缺少这些列时默认 `unspecified / in_service`，更新已有设备时空单元格保留原值。字段可写稳定英文值或规范中文值，导出固定使用稳定英文值。
...
- `operation_status` 描述业务投运状态，可选在用、未并网、调试中、暂停使用和已退役。设备管理默认查询 `in_service`，用户可切换“全部”或组合“二期 + 未并网”查看调试设备。
- 旧数据库幂等迁移只补列：全部现有设备默认 `project_phase=unspecified`、`operation_status=in_service`。迁移不扫描设备名称、编号、IP、站点或排序，不改变 ID、UUID、地址、凭据、任务和采集历史。
- 单台编辑、右键菜单和批量工具栏均可设置生命周期字段。批量更新使用一个 `BEGIN IMMEDIATE` 事务，目标 UUID 缺失或校验失败时整体回滚。投运状态变更可保存原因、更新时间和本机操作者标识。
- 未并网和调试中设备不在默认列表与自动候选中，但仍可按用户显式选择执行连接测试、详情刷新、配置采集和外部终端等手动调试。暂停使用同样允许手动调试并显示状态提示。已退役设备可查看、筛选和导出历史，创建新连接或采集前必须先改为调试中或在用。
- 自动候选调用必须显式传入 `operation_status=in_service`；普通 Repository 查询、按 UUID 详情、历史和导入匹配不得全局过滤。配置采集中心默认只列在用设备，并提供“包含未并网等非在用设备”手动调试开关。
- 二期正式并网时，筛选二期未并网设备并批量改为“在用”即可恢复默认显示和自动候选，原设备及历史关系保持不变。实际分期必须由用户选择，禁止按编号范围或 IP 自动判断。
