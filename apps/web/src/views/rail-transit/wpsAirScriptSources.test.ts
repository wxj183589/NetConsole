...
function smartSheetRuntime(initialNames: string[] = []) {
  type MockField = { id: string; name: string; type: string }
  type MockRecord = { id: string; fields: Record<string, unknown> }
  type MockSheet = Record<string, any> & {
    Id: string
    Name: string
    fields: MockField[]
    records: MockRecord[]
  ...
  function makeSheet(name: string, requestedFields: Array<{ name: string; type: string }> = []): MockSheet {
    const sheet = {
      Id: `sheet-${nextSheetId++}`,
      Name: name,
      fields: requestedFields.map((field) => ({ id: `field-${nextFieldId++}`, ...field })),
      records: [],
    } as MockSheet
    sheet.Field = {
      GetFields: () => sheet.fields.map((field) => ({ ...field })),
      CreateFields: ({ Fields }: { Fields: Array<{ name: string; type: string }> }) => {
        const created = Fields.map((field) => ({ id: `field-${nextFieldId++}`, ...field }))
        sheet.fields.push(...created)
        return created.map((field) => ({ ...field }))
      },
      UpdateFields: ({ Fields }: { Fields: Array<{ id: string; name?: string }> }) => {
        for (const update of Fields) {
          const field = sheet.fields.find((item) => item.id === update.id)
          if (field && update.name) field.name = update.name
        }
        return sheet.fields.map((field) => ({ ...field }))
      },
    }
    sheet.Record = {
      GetRecords: ({ Offset, PageSize = 100 }: { Offset?: string | number | null; PageSize?: number } = {}) => {
        const start = Number(Offset || 0)
        const records = sheet.records.slice(start, start + PageSize).map((record) => ({ id: record.id, fields: { ...record.fields } }))
        const next = start + records.length
        return { records, ...(next < sheet.records.length ? { offset: String(next) } : {}) }
      },
      CreateRecords: ({ Records }: { Records: Array<{ fields: Record<string, unknown> }> }) => {
        const created = Records.map((record) => ({ id: `record-${nextRecordId++}`, fields: { ...record.fields } }))
        sheet.records.push(...created)
        return created.map((record) => ({ id: record.id, fields: { ...record.fields } }))
      },
      UpdateRecords: ({ Records }: { Records: Array<{ id: string; fields: Record<string, unknown> }> }) => {
        for (const update of Records) {
          const record = sheet.records.find((item) => item.id === update.id)
          if (record) Object.assign(record.fields, update.fields)
        }
        return Records
      },
      DeleteRecords: ({ RecordIds }: { RecordIds: string[] }) => {
        const deleted = RecordIds.map((id) => ({ id, deleted: sheet.records.some((record) => record.id === id) }))
        for (const id of RecordIds) {
          const index = sheet.records.findIndex((record) => record.id === id)
          if (index >= 0) sheet.records.splice(index, 1)
        }
        return deleted
      },
    }
    sheet.View = { GetViews: () => [{ id: `view-${sheet.Id}`, name: '表格' }] }
    sheet.Move = ({ Before, After }: { Before?: string | null; After?: string | null }) => {
      const from = sheets.indexOf(sheet)
      if (from >= 0) sheets.splice(from, 1)
      if (Before) {
        const target = sheets.findIndex((item) => item.Id === Before)
        sheets.splice(Math.max(0, target), 0, sheet)
      } else if (After) {
        const target = sheets.findIndex((item) => item.Id === After)
        sheets.splice(target + 1, 0, sheet)
      } else {
        sheets.push(sheet)
      }
    }
  ...
  initialNames.forEach((name) => makeSheet(name))
  const application = {
    Sheet: {
      GetSheets: () => sheets,
      CreateSheet: ({ Name, Fields = [] }: { Name: string; Fields?: Array<{ name: string; type: string }> }) => makeSheet(Name, Fields),
    },
  }
  return {
    application,
    sheets,
    sheet: (name: string) => sheets.find((item) => item.Name === name),
    names: () => sheets.map((sheet) => sheet.Name),
  }
}

function executeSmartSheet(runtime: ReturnType<typeof smartSheetRuntime>, args: Record<string, unknown>) {
  const source = wpsAirScriptSource('wps_smart_sheet', 'sync')
  const raw = new Function('Application', 'Context', source)(runtime.application, { argv: args })
  return JSON.parse(String(raw)) as Record<string, any>
}

function executeSmartConnectionProbe(application: Record<string, unknown>, args: Record<string, unknown>) {
  const source = wpsAirScriptSource('wps_smart_sheet', 'probe')
  const raw = new Function('Application', 'Context', source)(application, { argv: args })
  return JSON.parse(String(raw)) as Record<string, any>
}

function smartSyncArgs(targetBatchId: string, value: string): Record<string, unknown> {
  const names = ['AP上线情况概览', '轨旁AP业务', '当前异常光衰', 'AP光衰处理记录', 'AP离线情况', 'AP离线台账', '新增上线AP概览', '待关联在线AP', '交换机光模块统计']
  return {
    operation: 'sync_trackside_ap_business',
    runtime_capability: 'VERIFIED',
    runtime_probe_verified: true,
    parent_batch_id: 'parent-smart',
    target_batch_id: targetBatchId,
    site_id: 'hzl10',
    business_key: 'rail_transit.trackside_ap_business',
    binding_id: 'wpsbind_v1_smart',
    initialize_binding: true,
    snapshot_revision: `revision-${targetBatchId}`,
    snapshot_sha256: `sha-${targetBatchId}`,
    record_batch_size: 20,
    smart_workbook: {
      sheets: names.map((name, index) => ({
        logical_sheet_key: index === 0 ? 'ap_online_history_overview' : `sheet-${index}`,
        sheet_name: name,
        sheet_order: index,
        sync_mode: index === 0 ? 'APPEND_SNAPSHOT' : 'FULL_REPLACE',
        fields: [
          { key: 'value', name: '业务值', type: 'MultiLineText' },
          { key: 'batch', name: '_NC_BATCH_ID', type: 'MultiLineText' },
          { key: 'row', name: '_NC_ROW_KEY', type: 'MultiLineText' },
          { key: 'revision', name: '_NC_REVISION', type: 'MultiLineText' },
        ],
        records: [{
          row_key: `${index}-${value}`,
          fields: {
            业务值: value,
            _NC_BATCH_ID: targetBatchId,
            _NC_ROW_KEY: `${index}-${value}`,
            _NC_REVISION: `revision-${targetBatchId}`,
          },
        }],
      })),
    },
...
describe('WPS AirScript deployment sources', () => {
  ...
  it('keeps smart-sheet writes behind the verified runtime capability probe', () => {
    ...
    expect(source).toContain('payload.smart_workbook')
    expect(source).toContain('sheetDto.sync_mode === "APPEND_SNAPSHOT"')
    expect(source).toContain('createRecordsBatched(sheet, expectedRecords, batchSize)')
    expect(source).toContain('function updateRecordsBatched(')
    expect(source).toContain('deleteRecordsBatched(sheet, oldIds, batchSize)')
    expect(source.indexOf('createRecordsBatched(sheet, expectedRecords, batchSize)'))
      .toBeLessThan(source.lastIndexOf('deleteRecordsBatched(sheet, oldIds, batchSize)'))
    expect(source).toContain('current.Move({ Before: null, After: sheetId(previous) })')
    expect(source).toContain('WPS_DOCUMENT_BINDING_MISMATCH')
    expect(source).not.toMatch(/Range\(|Value2|UsedRange|FreezePanes|ColumnWidth|Borders/)
  })

  it('verifies smart Sheet, Field and Record capabilities against an isolated probe sheet', () => {
    const runtime = smartSheetRuntime(['用户分析'])
    const result = executeSmartSheet(runtime, {
      operation: 'smart_runtime_write_probe',
      probe_id: 'probe-smart-1',
      site_id: 'hzl10',
      business_key: 'rail_transit.trackside_ap_business',
      binding_id: 'wpsbind_v1_smart',
    })
    ...
    expect(Object.values(result.core_capabilities)).toEqual(Array(9).fill(true))
    expect(runtime.names()).toEqual(['用户分析'])
  })

  it('keeps a structured smart capability failure when Sheet enumeration is unavailable', () => {
    const source = wpsAirScriptSource('wps_smart_sheet', 'sync')
    const raw = new Function('Application', 'Context', source)(
      { Sheet: {} },
      { argv: {
        operation: 'smart_runtime_write_probe',
        probe_id: 'probe-smart-unavailable',
        site_id: 'hzl10',
        business_key: 'rail_transit.trackside_ap_business',
        binding_id: 'wpsbind_v1_smart',
      } },
    )
    const result = JSON.parse(String(raw)) as Record<string, any>

    expect(result).toMatchObject({
      success: false,
      error_code: 'WPS_SMART_RUNTIME_PROBE_UNVERIFIED',
      runtime_capability: 'DEPLOYMENT_PENDING',
      binding_status: 'UNKNOWN',
      core_capabilities: { sheet_enum: false },
    })
    expect(result.capability_failures).toEqual(expect.arrayContaining([
      expect.objectContaining({ capability: 'sheet_enum' }),
    ]))
    expect(result.warnings).toEqual(expect.arrayContaining([
      expect.objectContaining({ capability: 'binding_readback' }),
    ]))
  })

  it('reads smart binding metadata without changing the workbook', () => {
    const runtime = smartSheetRuntime(['用户分析'])
    const metadata = runtime.application.Sheet.CreateSheet({
    ...
    metadata.Record.CreateRecords({
      Records: [
        { fields: { Key: 'document_id', Value: 'cbRdGQdb10R9' } },
        { fields: { Key: 'binding_id', Value: 'wpsbind_v1_smart' } },
        { fields: { Key: 'site_id', Value: 'hzl10' } },
        { fields: { Key: 'business_key', Value: 'rail_transit.trackside_ap_business' } },
        { fields: { Key: 'target_code', Value: 'wps_smart_sheet' } },
        { fields: { Key: 'target_type', Value: 'WPS_SMART_SHEET' } },
      ],
    })
    const before = JSON.stringify(runtime.sheets)

    const result = executeSmartConnectionProbe(runtime.application, {
    ...
    expect(JSON.stringify(runtime.sheets)).toBe(before)
  })

  it('keeps the smart connection probe successful when metadata readback is unavailable', () => {
    const result = executeSmartConnectionProbe({ Sheet: {} }, {
      operation: 'connection_test',
      site_id: 'hzl10',
      business_key: 'rail_transit.trackside_ap_business',
      binding_id: 'wpsbind_v1_smart',
  ...
  it('appends smart overview history, safely replaces eight business sheets and preserves user sheets', () => {
    const runtime = smartSheetRuntime(['用户分析'])
    const first = executeSmartSheet(runtime, smartSyncArgs('smart-1', '旧值'))
    const second = executeSmartSheet(runtime, smartSyncArgs('smart-2', '新值'))

    expect(first).toMatchObject({ success: true, sheet_count: 9, sheet_order_verified: true, binding_status: 'BOUND' })
    expect(second).toMatchObject({ success: true, sheet_count: 9, sheet_order_verified: true, binding_status: 'BOUND' })
    expect(runtime.names()).toContain('用户分析')
    const managedOrder = runtime.names().filter((name) => (smartSyncArgs('x', 'x').smart_workbook as any).sheets.some((sheet: any) => sheet.sheet_name === name))
    expect(managedOrder).toEqual(['AP上线情况概览', '轨旁AP业务', '当前异常光衰', 'AP光衰处理记录', 'AP离线情况', 'AP离线台账', '新增上线AP概览', '待关联在线AP', '交换机光模块统计'])
    expect(runtime.sheet('AP上线情况概览')?.records.map((record) => record.fields.业务值)).toEqual(['旧值', '新值'])
    for (const name of managedOrder.slice(1)) {
      expect(runtime.sheet(name)?.records.map((record) => record.fields.业务值)).toEqual(['新值'])
    }
    expect(runtime.sheet('_NetConsoleSyncMeta')?.records.length).toBeGreaterThan(0)
  })
