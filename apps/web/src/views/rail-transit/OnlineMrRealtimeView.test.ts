...
describe('Online MR realtime collection view', () => {
  it('shows only the current collection session and moves history to analysis', () => {
    ...
    expect(source).toContain('当前日志增长情况')
    expect(source).toContain('原始日志动态查看')
    expect(source).toContain('value="terminal_monitor"')
    expect(source).toContain('value="fping_raw"')
    expect(source).not.toContain('value="fping_v5"')
    ...
    expect(source).toContain('table-id="online-mr-raw-growth"')
...
describe('Online MR realtime mounted behavior', () => {
  ...
  it('lays out status and collapsed log growth side by side, then enables raw polling from an array collapse model', async () => {
    ...
    expect(wrapper.find('.runtime-grid').exists()).toBe(true)
    expect(wrapper.find('.runtime-status').exists()).toBe(true)
    expect(wrapper.find('.runtime-log-growth').exists()).toBe(true)
    ...
    expect(rawValues).toEqual(expect.arrayContaining(['terminal_monitor', 'mesh_link', 'wireless_status', 'channel_busy', 'iperf_client', 'fping_raw']))
    expect(rawValues).not.toContain('fping_v5')
    wrapper.unmount()
  })
})
