...
vi.mock('../../components/feedback/useConfirm', () => ({
  useConfirm: () => ({
    ...
    confirmChoice: mocks.confirmChoice,
  }),
}))
...
const PlanningStub = defineComponent({
  props: { modelValue: Array, stations: Array, editing: Boolean, readonly: Boolean, saving: Boolean },
  emits: ['update:modelValue', 'validation', 'generate'],
...
describe('轨道交通基础资料编辑闭环', () => {
  ...
  it('单个业务接口失败时显示部分刷新失败并保留已加载站点', async () => {
    ...
    expect(warning?.text()).toContain('连续失败：')
  ...
  it('局点切换事件会确认脏草稿并阻止保存中的切换', async () => {
    ...
    nativeConfirm.mockReturnValueOnce(false)
    const cancelled = new CustomEvent('netconsole:before-site-switch', { cancelable: true, detail: { targetSiteId: 'line-b' } })
    expect(window.dispatchEvent(cancelled)).toBe(false)
    expect(wrapper.find('input[placeholder="请输入线路名称"]').element).toHaveProperty('value', '待确认局点切换线路')

    nativeConfirm.mockReturnValueOnce(true)
    const accepted = new CustomEvent('netconsole:before-site-switch', { cancelable: true, detail: { targetSiteId: 'line-b' } })
    ...
    mocks.save.mockImplementationOnce(() => new Promise((resolve) => { finishSave = resolve }))
    await wrapper.find('input[placeholder="请输入线路名称"]').setValue('保存中局点切换线路')
    await button(wrapper, '保存').trigger('click')
    await vi.waitFor(() => expect(mocks.save).toHaveBeenCalledOnce())
    const saving = new CustomEvent('netconsole:before-site-switch', { cancelable: true, detail: { targetSiteId: 'line-c' } })
    expect(window.dispatchEvent(saving)).toBe(false)
    finishSave?.({ revision: 'c'.repeat(64), created_count: 0, updated_count: 1, deleted_count: 0, warnings: [], validation_issues: [] })
    await flushPromises()
    wrapper.unmount()
    if (originalConfirm) Object.defineProperty(window, 'confirm', { configurable: true, value: originalConfirm })
  ...
  it('只读状态只展示数据且不显示任何编辑入口', async () => {
    mocks.editSession.mockResolvedValue({
      ...writableSession,
      can_write: false,
      write_denial_code: 'READ_ONLY',
      write_denial_reason: '当前测试会话只读。',
    })
    const wrapper = await mountView()

    expect(wrapper.text()).toContain('设备管理 · 站点字段')
    expect(wrapper.findAll('button').some((item) => item.text().trim() === '编辑')).toBe(false)
    expect(wrapper.text()).not.toContain('从设备管理生成')
    expect(wrapper.text()).not.toContain('导入模板')
    expect(mocks.stationSourcePreview).not.toHaveBeenCalled()
    expect(mocks.save).not.toHaveBeenCalled()
    wrapper.unmount()
  })

  it('将设备来源应用到草稿，点击保存后才提交 validate 和 changes', async () => {
    const wrapper = await mountView()
  ...
  it('只读状态以多个标签展示轨道设施且不渲染模板导入控件', async () => {
    mocks.editSession.mockResolvedValue({
      ...writableSession,
      can_write: false,
      write_denial_code: 'READ_ONLY',
    ...
    expect(wrapper.find('input[accept=".xlsx"]').exists()).toBe(false)
    expect(wrapper.text()).not.toContain('导入模板')
    expect(mocks.save).not.toHaveBeenCalled()
    wrapper.unmount()
  })
  ...
  it('可写会话可编辑自动区间并记录人工覆盖，端点展示名不会进入保存值', async () => {
    mocks.stationsPage.mockResolvedValue({
      items: [
    ...
    expect(sectionTable.find('input[data-field="section-name"]').exists()).toBe(true)
  ...
  it('可写会话直接允许站点选择，批量删除预检不会静默跳过阻断项', async () => {
      ...
      items: [
        {
          ...
          references: { section_start_count: 0, section_end_count: 0, ap_count: 0, device_count: 0, relation_count: 0, endpoint_extension_count: 0, plan_count: 0, total_count: 0 },
        },
        {
          ...
          references: { section_start_count: 1, section_end_count: 0, ap_count: 1, device_count: 0, relation_count: 0, endpoint_extension_count: 0, plan_count: 0, total_count: 2 },
        },
      ],
    })
    const wrapper = await mountView()
