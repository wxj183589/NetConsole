...
import {
  ...
  downloadMeshAnalysisReport,
  downloadOnlineMrReport,
...
async function downloadArtifact(): Promise<void> {
  ...
  error.value = ''
  try {
    if (task.value.action === 'mesh_analysis_report') await downloadMeshAnalysisReport(task.value.artifact_id)
    else if (task.value.action === 'online_mr_report') await downloadOnlineMrReport(task.value.artifact_id)
...
onMounted(() => { void loadOnlineTrains(); void recoverTask() })
...
<template>
  <section class="rail-web-parity">
    ...
    <el-card shadow="never"><template #header>在线列车（{{ onlineTrains.length }}）</template><div class="row"><el-input v-model="trainId" placeholder="列车 ID（可选）" /><el-button :loading="loading" @click="loadOnlineTrains">重新加载</el-button><el-button type="primary" :loading="loading" @click="startCarCheck">开始车内通信检测</el-button></div><el-table :data="onlineTrains" height="280" empty-text="暂无在线列车"><el-table-column prop="train_no" label="列车" /><el-table-column prop="train_name" label="名称" /><el-table-column prop="communication_status" label="通信状态" /><el-table-column prop="current_mesh_links" label="MESH 链路" /><el-table-column prop="active_sessions" label="活动会话" /><el-table-column prop="warning_count" label="告警" /><el-table-column prop="last_updated_at" label="更新时间" /></el-table></el-card>
    <el-card shadow="never"><template #header>MR 原始日志导入</template><div class="row"><el-input v-model="mrId" placeholder="MR ID" /><el-input v-model="displayName" placeholder="显示名称" /><input type="file" multiple accept=".log,.txt" @change="selectFiles"><el-button type="primary" :loading="loading" :disabled="!selectedFiles.length || !mrId || !displayName" @click="importLogs">提交受控导入</el-button></div></el-card>
    <el-card shadow="never"><template #header>Online MR 指标与报告</template><div class="row"><el-input v-model="sessionId" placeholder="Session ID" /><el-button :loading="loading" @click="loadMetrics">读取指标</el-button><el-button :loading="loading" @click="exportReport">提交报告导出</el-button></div><el-table :data="metrics" empty-text="暂无指标"><el-table-column prop="metric_type" label="指标" /><el-table-column prop="series_key" label="序列" /><el-table-column prop="summary.count" label="点数" /></el-table></el-card>
    <el-card shadow="never"><template #header>离线 MESH 报告</template><div class="row"><el-input v-model="meshSessionId" placeholder="MESH Session ID" /><el-button :loading="loading" @click="exportMeshReport">提交报告导出</el-button></div></el-card>
