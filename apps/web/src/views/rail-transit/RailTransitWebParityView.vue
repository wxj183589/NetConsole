...
import { exportOnlineMrReport, importMeshAnalysis, listOnlineTrains, queryOnlineMrMetrics, startCarNetworkDiagnostic, refreshTracksideBusiness } from '../../api/railTransitWeb'
...
<template>
  <section class="rail-web-parity">
    ...
    <el-card shadow="never"><template #header>MR 原始日志导入</template><div class="row"><el-input v-model="mrId" placeholder="MR ID" /><el-input v-model="displayName" placeholder="显示名称" /><input type="file" multiple accept=".log,.txt,.csv" @change="selectFiles"><el-button type="primary" :disabled="!selectedFiles.length" @click="importLogs">提交受控导入</el-button></div></el-card>
    ...
    <el-alert v-if="task" type="info" :title="`${task.message} · ${task.task_id} · ${task.status}`" :closable="false" />
  </section>
</template>
