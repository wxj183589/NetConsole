...
describe('Electron-only packaging', () => {
  ...
  it('adds a separate, validated business-data-root page to the existing NSIS installer', () => {
    ...
    expect(script).toContain('--collect-host-profile')
    expect(script).toContain('--set-runtime-performance-mode')
    expect(script).toContain('StrCpy $NetConsolePerformanceMode "standard"')
    expect(script).not.toContain('NetConsoleRuntimeModePageCreate')
    expect(script).not.toContain('NetConsoleRuntimeModePageLeave')
    expect(script).not.toContain('NetConsoleRuntimeModeStandard')
    expect(script).not.toContain('NetConsoleRuntimeModeUnattended')
    expect(script).not.toContain('server_unattended')
    expect(script).not.toContain('服务器 / 无人值守优先')
    expect(script).not.toContain('Microsoft Windows NT\\CurrentVersion") "ProductName"')
  ...
  it('runs the packaged Electron smoke only in a unique D-drive test root', () => {
    ...
    expect(script).toContain("const WINDOWS_TEST_DATA_ROOT = resolve(projectRoot, '..', 'test-data', 'NetConsole')")
...
describe('Local Windows packaging launcher', () => {
  ...
  it('falls back to a process-local Corepack pnpm without changing the machine', () => {
    ...
    expect(localScript).toContain('[Environment]::SetEnvironmentVariable("PYTHONUTF8", "1", "Process")')
    expect(localScript).toContain('[Environment]::SetEnvironmentVariable("PYTHONIOENCODING", "utf-8", "Process")')
