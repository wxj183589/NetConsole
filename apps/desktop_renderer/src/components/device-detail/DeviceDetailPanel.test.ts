...
describe('DeviceDetailPanel', () => {
  ...
  it('uses backend section capabilities and shares one lazy detail presentation', () => {
    ...
    expect(source).toContain('sortableKeysBySection')
    expect(source).toContain("sortable: 'custom' as const")
    expect(source).toContain('@sort-change="handleSectionSortChange"')
    expect(source).toContain("sectionQuery.sort_by = sort.order ? prop : ''")
    expect(source).toContain("sort.order === 'descending' ? 'desc' : 'asc'")
    ...
    expect(source).toContain('const recentColumns = computed<NcTableColumn<DeviceDetailRecord>[]>')
    expect(source).toContain('getDeviceRecentChangeCounts')
    expect(source).toContain('最近变化：暂无')
    expect(source).toContain('最近变化加载中')
    expect(source).toContain('加载最近变化失败')
