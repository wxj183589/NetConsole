...
describe('GlobalTaskCenter', () => {
  ...
  it('deduplicates terminal notifications and separates foreground from native delivery', () => {
    expect(source).toContain('shouldNotifyTerminal')
    ...
    expect(source).toContain('const TASK_NOTIFICATION_DURATION')
    expect(source).toContain('success: 5000')
    expect(source).toContain('warning: 8000')
    expect(source).toContain('failure: 10000')
    expect(source).toContain('duration: TASK_NOTIFICATION_DURATION[kind]')
    expect(source).toContain('const MAX_VISIBLE_TASK_NOTIFICATIONS = 3')
