...
const canWrite = computed(() => isFeatureEnabled('capability.train_communication.point_table_write') && isFeatureEnabled('capability.rail_transit.task_control'))
  ...
  <el-dialog v-model="visible" title="在线列车车内通信点表" width="96vw" top="2vh" :close-on-click-modal="false" :before-close="closeDialog" destroy-on-close>
    <div class="dialog-body">
      ...
      <div class="toolbar">
        ...
        <el-select v-model="exportFormat" style="width:95px"><el-option label="XLSX" value="xlsx" /><el-option label="CSV" value="csv" /></el-select><el-button :disabled="!isFeatureEnabled('capability.train_communication.point_table_export') || taskRunning" @click="exportTable">导出</el-button>
      ...
      <div v-if="task" class="task-bar"><span>任务已提交，详细进度请查看任务中心</span><el-tag>{{ task.status }}</el-tag><span>{{ task.error_message || task.message }}</span><el-button @click="openTaskWindow">打开任务中心</el-button></div>
    </div>
