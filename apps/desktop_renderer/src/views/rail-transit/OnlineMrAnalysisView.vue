...
const canParse = computed(() => Boolean(detail.value?.has_raw_data && isFeatureEnabled('capability.online_mr.parse')))
const reportDisabled = computed(() => !parsedReady.value || !isFeatureEnabled('capability.online_mr.report_export'))
...
const desktopLocationAvailable = computed(() => Boolean(
  ...
  && isFeatureEnabled('capability.desktop_native_integration')
  && isFeatureEnabled('capability.online_mr.open_location'),
))
...
async function ensureParsedDatabaseCurrent(context = currentRequestContext()): Promise<void> {
  ...
  if (!selected || !isCurrent(context) || !isFeatureEnabled('capability.online_mr.parse')) return
...
async function deleteSession(targetSessionId: string): Promise<void> {
  ...
  if (!selected || !isFeatureEnabled('capability.online_mr.session_delete')) return
  ...
  <section class="analysis-page">
    <header class="page-heading">
      ...
      <div class="actions">
        ...
        <el-button data-testid="delete-session" type="danger" plain :icon="Delete" :loading="deletingSessionId === sessionId" :disabled="sessionActionsDisabled || sessionResourceBusy || Boolean(selectedDeleteBlockReason) || !isFeatureEnabled('capability.online_mr.session_delete')" :title="selectedDeleteBlockReason" @click="deleteCurrentSession()">删除</el-button>
      </div>
    </header>
        ...
        <el-tab-pane name="session-history" label="会话记录">
          <NcDataTable table-id="online-mr-analysis-session-history" route-key="/rail-transit/online-mr-analysis" :data="sessions" :columns="sessionColumns" row-key="session_id" :current-row-key="sessionId || ''" highlight-current-row border :height="tableHeight" empty-text="暂无会话" @row-click="selectSession">
            <template #cell-actions="{ row }">
              ...
              <el-button :data-testid="`row-delete-session-${row.session_id}`" link type="danger" :icon="Delete" :loading="deletingSessionId === row.session_id" :disabled="sessionResourceBusy || openingSessionId === row.session_id || Boolean(onlineMrSessionDeleteBlockReason(row)) || !isFeatureEnabled('capability.online_mr.session_delete')" :title="onlineMrSessionDeleteBlockReason(row)" @click.stop="deleteSession(row.session_id)">删除</el-button>
            </template>
          </NcDataTable>
        </el-tab-pane>
