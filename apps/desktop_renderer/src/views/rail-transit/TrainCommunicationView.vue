...
const canWritePointTable = computed(() => isFeatureEnabled('capability.train_communication.point_table_write') && isFeatureEnabled('capability.rail_transit.task_control'))
const pointTableDisabledReason = computed(() => canWritePointTable.value ? '' : '点表写入功能未启用')
const canStartBlockReason = computed(() => {
  if (!isFeatureEnabled('capability.train_communication.diagnostic_execute')) return '车内通信检测功能未启用'
  if (!isFeatureEnabled('capability.rail_transit.task_control')) return '统一任务中心未启用'
