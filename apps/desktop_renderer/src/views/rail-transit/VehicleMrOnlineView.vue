...
<template>
  <section class="vehicle-page">
    <header class="page-heading"><div><p class="eyebrow">RAIL TRANSIT · VEHICLE MR ONLINE</p><h1>列车在线情况</h1><p>连续采集 AC Mesh-Link，按 CT/TC 展示当前轨旁 AP、站点、RSSI、最后出现时间与映射策略。</p></div><div class="actions"><el-button :loading="loading" @click="loadTrains()">刷新页面</el-button><el-button :disabled="taskRunning || !selectedControllerId || !isFeatureEnabled('capability.train_online.refresh')" @click="refreshAll">刷新在线状态</el-button><el-button :disabled="taskRunning || !isFeatureEnabled('capability.train_online.refresh')" @click="refreshApMapping">刷新 AP 映射</el-button><el-button type="primary" :disabled="collectionRunning || !isFeatureEnabled('capability.train_online.mapping_write')" @click="openMappings">映射表管理</el-button></div></header>
    ...
    <div class="content-card collection-bar">
      ...
      <el-button type="primary" :disabled="taskRunning || !selectedControllerId || !isFeatureEnabled('capability.train_online.collect') || !isFeatureEnabled('capability.rail_transit.task_control')" @click="startCollection">开始</el-button>
