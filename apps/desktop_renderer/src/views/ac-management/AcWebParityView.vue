...
async function loadData(): Promise<void> {
  ...
  error.value = ''
  try {
    extensions.value = isFeatureEnabled('capability.ac.extensions') ? (await listAcExtensions()).items : []
  } catch (cause) {
...
async function recoverTask(): Promise<void> {
  if (!isFeatureEnabled('capability.ac.refresh')) return
...
onMounted(() => {
  void loadData()
  if (isFeatureEnabled('capability.ac.refresh')) void recoverTask()
})
...
<template>
  <section class="ac-web-parity">
    <header class="heading"><div><p class="eyebrow">AC WEB · BOUNDED ENTRY</p><h1>AP 扩展、轨旁规划与本地任务</h1><p>只展示本页已接入能力；在线概览、光衰详情和配置对比继续复用既有成熟页面。</p></div><div class="actions"><el-button :loading="taskBusy" :disabled="!isFeatureEnabled('capability.ac.extensions.export') || !isFeatureEnabled('capability.ac.refresh')" @click="exportExtensions">导出扩展信息</el-button><el-button :loading="loading" @click="loadData">重新加载本地资料</el-button></div></header>
    ...
    <div class="toolbar"><el-input v-model="targetId" placeholder="当前局点 AC UUID" /><template v-if="isFeatureEnabled('capability.ac.dangerous_actions')"><el-select v-model="actionId"><el-option label="固化新 AP" value="persist_auto_ap" /><el-option label="开启 AP 远程登录" value="enable_ap_remote_login" /></el-select><el-button @click="createPlan">生成命令预览</el-button><el-button :disabled="!actionPlan" @click="confirmPlan">二次确认</el-button><el-button type="danger" :disabled="actionPlan?.status !== 'CONFIRMED'" @click="executePlan">执行真实任务</el-button></template><el-button :loading="taskBusy" :disabled="!isFeatureEnabled('capability.ac.refresh')" @click="rebuild('optical')">本地重算光衰视图</el-button></div>
    <el-alert type="warning" title="本地重算只读取当前数据库与缓存，不连接真实 AC。" show-icon :closable="false" />
    <el-alert v-if="isFeatureEnabled('capability.ac.dangerous_actions')" type="error" title="AC 写操作会连接真实设备；必须先核对固定命令预览并完成二次确认。" show-icon :closable="false" />
    <el-alert
    ...
    <div class="grid">
      <el-card shadow="never"><template #header>AP 扩展导入预览 / 回滚</template><input type="file" accept=".csv,.xlsx" :disabled="!isFeatureEnabled('capability.ac.extensions.preview')" @change="chooseFile"><el-alert v-if="extensionPreview" class="preview" type="info" :title="`${extensionPreview.file_name} · ${extensionPreview.row_count} 行 · 摘要 ${extensionPreview.preview_digest}`" :closable="false" /><div class="actions"><el-button type="primary" :loading="taskBusy" :disabled="!extensionPreview || !isFeatureEnabled('capability.ac.extensions.apply')" @click="applyExtension">确认写入</el-button><el-button :loading="taskBusy" :disabled="!lastAuditId || !isFeatureEnabled('capability.ac.extensions.rollback')" @click="rollbackExtension">回滚最近导入</el-button></div></el-card>
