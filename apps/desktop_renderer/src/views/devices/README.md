...
- 新数据库默认 `project_phase=unspecified / work_scope_status=included`。旧数据库加法迁移保留 `operation_status*` 列并复制到新字段；已知旧状态使用明确映射，未知状态阻止迁移。迁移不扫描设备名称、编号、IP、站点或排序，不改变 ID、UUID、地址、凭据、任务和采集历史。
