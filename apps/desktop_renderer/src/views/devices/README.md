...
设备主列表、能力事实、详情分区和历史表格已迁移到 `NcDataTable`。表头/内容默认居中，描述和错误等长文本按列类型明确左对齐；列宽由公共文本测量、字段类型基线和可视区剩余空间共同计算，分区/历史偏好按 section/kind 隔离。设备主列表的正式 Electron 偏好键为 `device-management.device-list`，与设备详情各分区键隔离；浏览器开发模式使用相同公共实现的 localStorage 回退。设备主列表由系统名、分组、站点和主备地址共同拉伸，站点优先；复选框、登录协议、时间、状态和操作列保持边界。页面不再散写 Element Plus 列宽、`header-align`、`measureText()` 或平均分配算法。
...
设备主列表的默认排序由 Backend Service 在完整筛选结果上执行，固定为
`COCC > BOCC > 车站 > 车载-MR > 车载-SW > 其它分组 > 未分组`，组内按设备名称自然排序，
再执行分页。`车载-3SW` 等历史分组名只在排序比较时归一化为车载-SW，原始显示名称和数据库值不变。
用户通过排序控件选择其他字段后使用对应的显式排序；刷新页面重新使用默认排序，Renderer 不对当前页二次排序。
...
- 正式 33 列模板和导出在既有设备身份列后增加“建设阶段”“当前工作状态”“当前工作状态说明”；上一版 33 列“投运状态”模板及 30/28/21 列文件仍可导入。新建设备缺少这些列时默认 `unspecified / included`，更新已有设备时空单元格保留原值。字段可写稳定英文值或规范中文值，导出固定使用稳定英文值。
...
- `project_phase` 只描述建设归属，可选一期、二期、三期、其他和未指定；它不直接决定设备是否显示或采集。
- `work_scope_status` 只描述设备是否参加当前调试、采集和管理范围，可选 `included`（参与当前调试）与 `excluded`（暂不参与）。它不表示设备真实投运、连接、凭据或采集状态。
- 新建设备默认 `project_phase=phase_1 / work_scope_status=included`。已有数据库中的 `unspecified` 保留，需人工确认，不做全库自动迁移。旧数据库加法迁移保留 `operation_status*` 列并复制到新字段；已知旧状态使用明确映射，未知状态阻止迁移。迁移不扫描设备名称、编号、IP、站点或排序，不改变 ID、UUID、地址、凭据、任务和采集历史。
- 单台编辑、右键菜单和批量工具栏均可设置建设阶段与当前工作状态。批量更新使用一个 `BEGIN IMMEDIATE` 事务，目标 UUID 缺失或校验失败时整体回滚。工作状态变更可保存原因、更新时间和本机操作者标识。
- 设备管理和配置采集默认查询 `included`；用户切换“全部”或组合“一期 + 暂不参与”后仍可找到 `excluded` 设备，并可明确发起连接测试、详情刷新、配置采集和外部终端等手动操作。
- 自动候选调用必须显式传入 `work_scope_status=included`，执行前按同一事实复核；普通 Repository 查询、按 UUID 详情、历史和导入匹配不得全局过滤。建设阶段与当前工作状态可以独立组合，禁止按编号范围或 IP 自动推断。
...
- ZTE ZXR10 设备详情 Profile v3 固定执行版本、接口、VLAN、`show opticalinfo brief` 和两条 LLDP 只读命令，不下发未经确认的关闭分页命令，也不回退执行 H3C `screen-length` / `display` 命令。普通刷新默认不逐端口追加 `show opticalinfo <safe-interface>`；该命令仅保留为显式 opt-in 的受控诊断能力。采集先用 `show version` 确认 C89E 或 59X/5960X-ES，其他 ZXR10 型号在后续命令前失败关闭。
- ZTE 光模块状态统一为 `no_module / no_light / abnormal / normal`：模块离线时清空全部身份和功率字段；`Unknown` 或 RX 缺失时为 `no_light`，原因固定为“设备未返回接收光功率”；异常只使用设备返回的 RX/TX 低高阈值并按严格越界判断，等于低阈值仍正常，不进入通用 `-35 dBm` 兜底。旧快照在 Repository 读出时也使用同一规则；H3C 命令和状态规则不变。
...
设备管理页记录当次批量连接测试的 Task ID；任务进入完成、失败、取消或中断终态后，页面自动重新读取当前分页并保留筛选和选择状态，使“连接状态”和“最后测试时间”无需手工刷新即可更新。
