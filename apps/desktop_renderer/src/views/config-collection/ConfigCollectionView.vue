  ...
  <section class="config-collection">
    ...
    <div class="toolbar content-card">
      ...
      <div class="toolbar-actions">
        ...
        <el-button :disabled="!isFeatureEnabled('capability.config_collection.open_directory')" @click="openResultDirectory('config_snapshots')">快照目录</el-button>
        <el-button :disabled="!isFeatureEnabled('capability.config_collection.open_directory')" @click="openResultDirectory('config_exports')">导出目录</el-button>
        <el-button :disabled="!isFeatureEnabled('module.task_center')" @click="openTaskWindow">任务中心</el-button>
      </div>
    </div>
    ...
    <div class="main-grid">
      <div class="content-card device-card">
        <div class="card-heading"><div><h2>设备选择</h2><p>共 {{ devicePage.total }} 台 H3C 设备</p></div><div class="heading-actions"><el-button type="primary" :disabled="!selectedDevices.length || hasActiveTasks || !isFeatureEnabled('capability.config_collection.fetch')" @click="collectSelected">采集 running / saved</el-button><el-button :disabled="!selectedDevices.length || hasActiveTasks || !isFeatureEnabled('capability.config_collection.save_force')" @click="saveSelected">保存配置</el-button><el-button :disabled="selectedDevices.length !== 2 || hasActiveTasks || !isFeatureEnabled('capability.config_collection.diff')" @click="compareDevices">比较两台设备</el-button></div></div>
        <NcDataTable
          v-loading="loading"
      ...
      <div class="content-card snapshot-card">
        <div class="card-heading"><div><h2>快照历史</h2><p>{{ selectedDevice?.name || '请选择设备' }} · 左右选择在切换设备或类型后仍保留</p></div><div class="heading-actions"><el-select v-model="snapshotType" clearable placeholder="配置类型" @change="loadSnapshots"><el-option label="运行配置" value="running" /><el-option label="保存配置" value="saved" /><el-option label="差异" value="diff" /></el-select><el-button :disabled="!selectedDevice || !isFeatureEnabled('capability.config_collection.diff')" @click="compareLatest">最新差异</el-button><el-button :disabled="!selectedSnapshots.length || !isFeatureEnabled('capability.config_collection.export')" @click="exportSelectedSnapshots">导出 ZIP</el-button><el-button :icon="Delete" :disabled="!selectedSnapshots.length || !isFeatureEnabled('capability.config_collection.delete')" @click="deleteSelectedSnapshots">删除历史</el-button></div></div>
        <div class="comparison-basket" aria-label="配置快照左右选择篮">
          ...
          <div class="comparison-actions"><el-button type="primary" :disabled="!hasValidSnapshotPair || !isFeatureEnabled('capability.config_collection.diff')" @click="compareSnapshots">比较左右快照</el-button><el-button :disabled="!hasValidSnapshotPair || !isFeatureEnabled('capability.config_collection.export')" @click="exportSelectedDiff">导出左右差异</el-button></div>
        </div>
        <NcDataTable
