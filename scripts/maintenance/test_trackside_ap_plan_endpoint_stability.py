...
TEST_DATA_ROOT = Path(r"D:\NetConsoleTestData")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help=r"新的隔离 TEST 数据根（Windows 必须位于 D:\NetConsoleTestData\<run-id>）",
    )
...
def _validate_paths(source: Path, test_root: Path) -> tuple[Path, Path]:
    ...
    if os.name == "nt":
        ...
        if test_root == allowed or not test_root.is_relative_to(allowed):
            raise ValueError(r"--test-root 必须位于 D:\NetConsoleTestData\<run-id>，且不能直接使用测试根")
