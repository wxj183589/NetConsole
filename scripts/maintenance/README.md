...
- `cleanup_retired_tasks.py`：默认只读预览 GUI 已从任务中心移除的 terminal task；显式 `--apply` 才复用 Task Center Operational GC 删除 tasks.db 自有 rows，不删除日志、Artifact、业务数据或当前映射。`--data-root` 被 canonical runtime marker 标记为 Production 时，`--apply` 还必须通过正式 SiteRegistry/allowlist scope guard；未登记局点、别名和路径输入均在写库前拒绝。
