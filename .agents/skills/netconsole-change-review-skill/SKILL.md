...
- 本地统一执行入口是 `python -m scripts.quality.local_gate`：`--mode auto` 按风险选择 FAST/CONSUMER/FULL，`--mode consumer` 执行 Registry 套件，`--mode full` 执行最终平台基线。Gate 会强制 `RuntimeMode.TEST`、唯一 `D:/NetConsoleTestData/<run-id>` 和 `.local-reports/` 报告。
