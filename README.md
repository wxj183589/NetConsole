...
`pnpm dev:codex` 和 `pnpm smoke:dev` 使用 `D:\study\NetConsole-Workspace\test-data\NetConsole\<run-id>` 下的临时测试根，不读取正式业务根。需要持久保存开发数据且机器已配置数据根时，才使用 `pnpm dev`。Python 定向测试：
