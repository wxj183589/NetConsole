...
`pnpm dev:codex` 和 `pnpm smoke:dev` 使用 `<repository-parent>\test-data\NetConsole\<run-id>` 下的临时测试根，不读取正式业务根。需要持久保存开发数据且机器已配置数据根时，才使用 `pnpm dev`。Python 定向测试：
...
本地 Gate 使用唯一 `<repository-parent>\test-data\NetConsole\<run-id>`，不会读取正式业务根；GitHub Actions 使用相同的路径推导规则。后续 Gate 以主机/副机本地定向测试、主机集成验证、必要的真实 `D:\NetConsoleData-dev` 验收和 Release Gate 为准。
