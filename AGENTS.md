...
- NetConsole 面向现场运维。在不可能造成设备错误配置、凭据泄露等实质风险的情况下，运维便利性和自动恢复优先于复杂安全交互；安全机制应尽量后台化、自动化，不应增加不必要的用户操作步骤。SSH/SFTP Host Key 采用统一 managed 事实源自动登记/替换，未知厂商和真正的认证、可达性、凭据或错误命令风险仍必须失败。
