...
def test_bundle_is_bounded_redacted_and_integrity_checked(tmp_path, monkeypatch) -> None:
    ...
    (logs / "ground.log").write_text(
        "token=TOP_SECRET password=bad\nnormal\n", encoding="utf-8"
    )
    raw.write_text(
...
def test_bundle_requires_current_site_scope_and_raw_is_opt_in(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(bundle, "detect_storage_profile", _storage)
    monkeypatch.setattr(bundle, "_windows_storage", lambda _root: {"status": "unavailable"})
    monkeypatch.setattr(bundle, "_raid_tools", lambda: {"status": "not_available"})
    other_raw = (
    ...
    other_raw.write_text('{"token":"OTHER_SITE_SECRET"}\n', encoding="utf-8")
    output = tmp_path / "diagnostic.zip"

    bundle.collect_field_diagnostic_bundle(
        {"data_root": str(tmp_path), "site_name": "demo", "sample_duration_minutes": 0},
        output,
    )

    with zipfile.ZipFile(output) as archive:
        assert "samples/raw_sample.ndjson" not in archive.namelist()
        assert "OTHER_SITE_SECRET" not in b"".join(archive.read(name) for name in archive.namelist()).decode(
            "utf-8", "replace"
        )


def test_bundle_rejects_site_path_escape(tmp_path) -> None:
    with pytest.raises(ValueError, match="局点范围无效"):
        bundle.collect_field_diagnostic_bundle(
            {"data_root": str(tmp_path), "site_name": "../other", "sample_duration_minutes": 0},
