...
def test_resolver_finds_development_tools_from_project_root(tmp_path: Path) -> None:
    fping = _write_tool(tmp_path / "tools" / "fping_v5" / "fping.exe")
    iperf = _write_tool(tmp_path / "tools" / "iperf" / "iperf3.exe")
    paths = PathResolver(tmp_path)

    assert resolve_tool_path("fping_v5", paths, project_root=tmp_path) == fping.resolve()
...
def test_resolver_finds_packaged_internal_tools(tmp_path: Path) -> None:
    ...
    fping = _write_tool(app_root / "_internal" / "tools" / "fping_v5" / "fping.exe")
    iperf = _write_tool(app_root / "_internal" / "tools" / "iperf" / "iperf3.exe")
    paths = PathResolver(app_root)

    assert resolve_tool_path("fping_v5", paths, project_root=tmp_path / "missing") == fping.resolve()
...
def test_resolver_prefers_custom_settings_path(tmp_path: Path) -> None:
    ...
    packaged = _write_tool(app_root / "_internal" / "tools" / "fping_v5" / "fping.exe")
    ...
    assert resolve_tool_path("fping_v5", paths, settings=settings, project_root=tmp_path / "missing") == custom.resolve()
...
def test_resolver_candidate_order_contains_packaged_dev_and_legacy_paths(tmp_path: Path) -> None:
    ...
    candidates = candidate_tool_paths("fping_v5", paths, project_root=tmp_path / "project_root")
    normalized = [path.as_posix() for path in candidates]

    assert normalized[0].endswith("NetConsole/_internal/tools/fping_v5/fping.exe")
    assert normalized[1].endswith("NetConsole/tools/fping_v5/fping.exe")
    assert normalized[2].endswith("NetConsole/tools/fping_v5/fping.exe") or normalized[2].endswith("NetConsole/_internal/tools/fping_v5/fping.exe") or normalized[2].endswith("tools/fping_v5/fping.exe")
    assert any(path.endswith("project_root/tools/fping_v5/fping.exe") for path in normalized)


def test_resolver_finds_nuitka_onefile_extracted_tools(tmp_path: Path, monkeypatch) -> None:
    package_root = tmp_path / "onefile"
    fping = _write_tool(package_root / "tools" / "fping_v5" / "fping.exe")
    ...
    assert resolve_tool_path("fping_v5", PathResolver(tmp_path / "app"), project_root=tmp_path / "missing") == fping.resolve()
