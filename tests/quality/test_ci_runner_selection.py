...
def test_windows_runner_selection_is_consistent_and_safe_for_public_prs() -> None:
    for path in WORKFLOWS:
        ...
        assert "D:\\study\\NetConsole-Workspace\\test-data\\NetConsole" in raw
        assert "NETCONSOLE_RUNTIME_MODE: test" in raw
        assert "NETCONSOLE_DATA_ROOT: D:\\study\\NetConsole-Workspace\\test-data\\NetConsole\\github-actions\\${{ github.run_id }}-${{ github.run_attempt }}" in raw
...
def test_manual_runner_choice_and_setup_documentation_are_present() -> None:
    ...
    for expected in (
        ...
        "D:\\study\\NetConsole-Workspace\\test-data\\NetConsole\\<run-id>",
        "pull_request",
    ):
