...
- commit：`02fde6e9c9487b7d8edee2a0ff556f56c0f3d847`
- branch：`main`
...
| 功能 | 优化前 | 当前 | 结果 |
| --- | --- | --- | --- |
| Site Switch | 8.5–14.4s，backend restart | 最终 commit 启动 interactive 6.03s；完整切换本轮未重跑；历史 warm metadata 362–453ms | PARTIAL |
| FIT-AP | 静态审计疑似 N+1 | 两个主测局点 page 164–316ms；批量明细生效 | PASS |
| Trackside snapshot | 72.4s 导出 snapshot 基线 | 直接快照杭州 1.31s、宁波12 8.73s；历史解码仍为热点 | PARTIAL |
| Trackside export | 约 77.4s | 约 74.8s；snapshot build 69.7s | PARTIAL |
| MESH table | 11.59s，heap 3.22GB | link page 0.65s，1,000 行服务端分页 | PASS（GUI 滚动待补） |
| MESH chart | profiling 约 17.06s | 2.02s，payload 2.22MB，128 series/3,319 points | PASS（GUI long-task 待补） |
