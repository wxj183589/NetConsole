...
def import_apply(request: Request, payload: ImportApplyRequestDTO) -> ImportApplyResultDTO:
    ...
    return ImportApplyResultDTO(
        ...
        imported_rows=int(audit.get("imported_rows") or 0),
        created_rows=int(audit.get("created_rows") or 0),
        updated_rows=int(audit.get("updated_rows") or 0),
        unchanged_rows=int(audit.get("unchanged_rows") or 0),
        warning_rows=int(audit.get("warning_rows") or 0),
        skipped_conflict_rows=int(audit.get("skipped_conflict_rows") or 0),
        skipped_invalid_rows=int(audit.get("skipped_invalid_rows") or 0),
        unmatched_fit_ap_rows=int(audit.get("unmatched_fit_ap_rows") or 0),
...
def _query(callback):
    with map_api_errors("轨道交通基础资料数据库暂时不可读"):
        try:
            return callback()
        except ValueError as exc:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=str(exc)) from exc
