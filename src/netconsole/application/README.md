...
刷新使用稳定 Operation ID `device.inventory.collect`，应用层不拼命令、不识别设备版本，也不直接访问 SQLite。未知厂商、角色、平台或 major family 必须由下层服务失败关闭；未登记 Patch/Release 不单独阻断已确认的 H3C Comware V7 family。H3C `wireless_controller` 通过 Comware family capability Profile 刷新设备事实、接口、光模块和 LLDP；Release-specific Profile 只作为 override。AC 业务关联继续只读复用 AC Query Service，FIT-AP/Radio/受控动作不进入设备详情用例。H3C MR 关联信息只读复用 Online MR Query Service，基础设备详情刷新走独立 `mobile_router` Profile，不能被通用交换机刷新接管。
...
参见 [设备管理页面](../../../apps/desktop_renderer/src/views/devices/README.md)、[业务服务](../services/README.md)、[开发规则](../../../docs/DEVELOPMENT_RULES.md)、[API/Application 边界](../../../docs/development/API_APPLICATION_BOUNDARY.md)和[Job Center](../../../docs/job-center/README.md)。
