...
刷新使用稳定 Operation ID `device.inventory.collect`，应用层不拼命令、不识别设备版本，也不直接访问 SQLite。未知或未验证厂商、角色、平台、Command Profile 必须由下层服务失败关闭。H3C AC/MR 的业务关联只读复用各自 Query Service，不能被通用交换机刷新接管。
