...
def parse_transceiver_diagnosis(output: str) -> list[dict[str, object | None]]:
    ...
    for raw_line in (output or "").splitlines():
        ...
        _set_if_match(current, "rx_low_alarm", line, r"(?i)Receive\s+power\s+low\s+alarm\s*[:\uff1a]\s*(.+)")
        _set_if_match(current, "rx_high_alarm", line, r"(?i)Receive\s+power\s+high\s+alarm\s*[:\uff1a]\s*(.+)")
        _set_if_match(current, "rx_low_warning", line, r"(?i)(?:Rx|Receive\s+power)\s+low\s+warning\s*[:\uff1a]\s*(.+)")
        _set_if_match(current, "rx_high_warning", line, r"(?i)(?:Rx|Receive\s+power)\s+high\s+warning\s*[:\uff1a]\s*(.+)")
        ...
        _set_if_match(current, "tx_low_warning", line, r"(?i)Tx low warning\s*[:\uff1a]\s*(.+)")
        _set_if_match(current, "tx_high_warning", line, r"(?i)Tx high warning\s*[:\uff1a]\s*(.+)")
        _set_if_match(current, "status", line, r"(?i)Status\s*[:\uff1a]\s*(.+)")
    if current:
        modules.append(current)
    return modules
