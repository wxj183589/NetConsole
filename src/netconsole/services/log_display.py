...
LOG_EVENT_ZH_MAP = {
    ...
    "BOOT_MAIN_WINDOW_CREATED": "主窗口创建完成",
    "BOOT_MAIN_WINDOW_SHOWN": "主窗口显示完成",
    "BOOT_BACKGROUND_TASKS_STARTED": "后台启动任务已安排",
    "BOOT_MODULE_LAZY_LOAD_STARTED": "模块懒加载开始",
    "BOOT_MODULE_LAZY_LOAD_FINISHED": "模块懒加载完成",
    "STARTUP": "启动流程",
    "SITE_LOADED": "局点加载完成",
...
def display_log_detail(detail: str) -> str:
    ...
    for match in _DETAIL_TOKEN_RE.finditer(text):
        ...
        replacements.append(
            (match.start(), match.end(), f"{DETAIL_KEY_ZH_MAP.get(key, key)}={DETAIL_VALUE_ZH_MAP.get(value, value)}")
        )
    if not replacements:
