...
class DiagnosticRedactor:
    ...
    ALLOWLIST = frozenset({
        "data_root", "storage_profile", "profile_source", "udp_port", "queue_capacity",
    ...
    def text(value: str) -> str:
        result = value
        import re
        result = re.sub(r"(?i)(password|passwd|secret|token|api[_-]?key|authorization|cookie|community)\s*([:=])\s*[^\s,;]+", r"\1\2[REDACTED]", result)
        return result
...
def _windows_storage(root: Path) -> dict[str, Any]:
    ...
    command = (
        ...
        "$v=Get-CimInstance -Namespace root/Microsoft/Windows/Storage -ClassName MSFT_VirtualDisk -ErrorAction SilentlyContinue; "
        "$f=Get-CimInstance -Namespace root/Microsoft/Windows/Storage -ClassName MSFT_PhysicalDisk -ErrorAction SilentlyContinue; "
        "@{partition=$p;disk=$d;virtual_disk=$v;physical_disks=$f} | ConvertTo-Json -Depth 5"
    )
...
def _file_inventory(root: Path, should_cancel: CancelCallback | None) -> tuple[dict[str, Any], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    inventory = {"status": "ok", "root": str(root), "total_bytes": total, "file_count": count, "top_level_bytes": directories}
...
def _collect_logs(root: Path, log_window_minutes: int, should_cancel: CancelCallback | None) -> tuple[dict[str, bytes], list[str]]:
    now = time.time()
    selected: list[Path] = []
    for candidate_root in (root / "logs", root / "files", root):
        if not candidate_root.exists():
            continue
        try:
            for path in candidate_root.rglob("*.log"):
                if path.is_file() and not path.is_symlink() and (now - path.stat().st_mtime <= log_window_minutes * 60 or path_root_name(path) == "logs"):
    ...
    for index, path in enumerate(selected):
        ...
        if reason:
            warnings.append(f"log:{_safe_rel(path, root)}: {reason}")
            continue
        outputs[f"logs/{('netconsole_recent.log' if index == 0 else 'backend_recent.log' if index == 1 else 'ground_recent.log')}"] = DiagnosticRedactor.text(data.decode("utf-8", errors="replace")).encode("utf-8")
    return outputs, warnings


def path_root_name(path: Path) -> str:
    return path.parts[-2].lower() if len(path.parts) > 1 else ""
...
def _raw_sample(root: Path, should_cancel: CancelCallback | None) -> tuple[bytes, dict[str, Any]]:
    ...
    return payload, {"status": "ok", "records": len(lines), "bytes": len(payload), "source_files": len(candidates)}
...
def _performance_snapshot(root: Path, duration_minutes: int, should_cancel: CancelCallback | None, progress: ProgressCallback | None) -> tuple[bytes, bytes, list[str]]:
    ...
    for index in range(samples):
        if _cancelled(should_cancel):
            raise DiagnosticCancelled("诊断采集已取消")
        disk, disk_reason = _run_powershell("Get-Counter '\\PhysicalDisk(*)\\Avg. Disk sec/Read','\\PhysicalDisk(*)\\Avg. Disk sec/Write','\\PhysicalDisk(*)\\Disk Reads/sec','\\PhysicalDisk(*)\\Disk Writes/sec','\\PhysicalDisk(*)\\Current Disk Queue Length' | Select-Object -ExpandProperty CounterSamples | Select-Object InstanceName,Path,CookedValue | ConvertTo-Json -Depth 3")
        if disk is None:
        ...
        for item in disk_items:
            if isinstance(item, Mapping):
                disk_rows.append({"sample": index, "data_root": str(root), **dict(item)})
    ...
    disk_headers = ("sample", "data_root", "InstanceName", "Path", "CookedValue")
...
def collect_field_diagnostic_bundle(params: Mapping[str, Any], output_path: str | Path, *, progress_callback: ProgressCallback | None = None, should_cancel: CancelCallback | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if isinstance(storage_data, Mapping):
            ...
            add_json("storage/physical_disks.json", storage_data.get("physical_disks") or _section("unavailable", reason="硬 RAID 可能隐藏物理盘"))
        else:
            for name in ("volumes.json", "disks.json", "physical_disks.json"):
                add_json(f"storage/{name}", windows_storage)
        add_json("storage/raid_controller.json", _raid_tools())
        add_json("storage/storage_mapping.json", {"data_root": str(root), "volume": detection.volume, "disk": None, "confidence": "LOW", "reason": "PhysicalDisk 映射依赖现场 Windows CIM"})
    except Exception as exc:
        failed_sections.append("storage")
        warnings.append(f"storage: {type(exc).__name__}: {exc}")
    runtime = params.get("runtime_snapshot") if isinstance(params.get("runtime_snapshot"), Mapping) else {}
    add_json("runtime/ground_health.json", _section(data=_redact(runtime.get("ground_health") if isinstance(runtime, Mapping) else None)) if runtime else _section("unavailable", reason="导出进程未连接 Ground 内存状态"))
    for name, key in (("udp_syslog.json", "udp"), ("raw_writer.json", "raw_writer"), ("parser.json", "parser"), ("sqlite.json", "sqlite"), ("spool.json", "spool"), ("task_runtime.json", "task_runtime")):
        value = runtime.get(key) if isinstance(runtime, Mapping) else None
        add_json(f"runtime/{name}", _section(data=_redact(value)) if isinstance(value, Mapping) else _section("unavailable", reason="当前运行摘要未提供"))
    ...
    try:
        inventory, largest, active = _file_inventory(root, should_cancel)
    ...
    if bool(params.get("raw_sample", True)):
        raw, raw_meta = _raw_sample(root, should_cancel)
        ...
        add_json("samples/raw_sample_meta.json", raw_meta)
    log_files, log_warnings = _collect_logs(root, int(params.get("log_window_minutes") or 30), should_cancel)
    files.update(log_files)
    warnings.extend(log_warnings)
    disk_csv, process_csv, perf_warnings = _performance_snapshot(root, int(params.get("sample_duration_minutes") or 0), should_cancel, progress_callback)
    ...
    summary = {
        "storage": {"data_root": str(root), "volume": str(root.drive or ""), "media_type": None, "confidence": None, "profile": None, "profile_source": None},
        "udp": runtime.get("udp") if isinstance(runtime, Mapping) else None,
        "parser": runtime.get("parser") if isinstance(runtime, Mapping) else None,
        "sqlite": runtime.get("sqlite") if isinstance(runtime, Mapping) else None,
        "spool": runtime.get("spool") if isinstance(runtime, Mapping) else None,
        "process": None,
    }
    try:
        ...
        pass
    manifest = {"bundle_version": "1", "created_at": created_at, "app_version": APP_VERSION, "git_revision": str(params.get("git_revision") or ""), "host_name": socket.gethostname(), "os_version": platform.platform(), "architecture": platform.machine(), "data_root": str(root), "site_id": site_name, "site_name": site_name, "collection_window": {"sample_duration_minutes": int(params.get("sample_duration_minutes") or 0), "log_window_minutes": int(params.get("log_window_minutes") or 30)}, "redaction_policy": "allowlist-config; bounded-tail-samples", "included_sections": sorted(files), "failed_sections": failed_sections, "warnings": warnings}
...
__all__ = ["DiagnosticCancelled", "DiagnosticRedactor", "collect_field_diagnostic_bundle"]
