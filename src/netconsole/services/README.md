...
设备详情当前为 `device.inventory.collect` 登记 H3C Comware `switch`、`wireless_controller` 和 `mobile_router` 通用只读 Profile。执行边界是 H3C + 已支持角色 + Comware major family 且 Profile 明确可执行；未知/未验证厂商、角色、平台或 major 不得回退执行 H3C 命令。H3C Comware V7 未登记 Patch/Release 不单独阻断 family 匹配，已证实差异才使用 Release override。H3C AC 的 FIT-AP/Radio/Mesh 仍由 AC 领域 Service 负责；MR 关联信息只读复用 Online MR Query Service，基础设备详情刷新使用独立 mobile_router Profile。Huawei/ZTE 当前仍只支持已入库快照展示和接口归一化，不代表命令已验证。真实设备状态保持 `REAL_DEVICE_PENDING`。
...
- [Job Center](../../../docs/job-center/README.md)
- [导出规范](../../../docs/export/PROCESS_POLICY.md)
