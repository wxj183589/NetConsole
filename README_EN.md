...
`pnpm dev:codex` and `pnpm smoke:dev` use a temporary `<repository-parent>\test-data\NetConsole\<run-id>` root and do not read the persistent application root. Use `pnpm dev` only when the machine has a configured persistent data root and you intend to retain development data. Example targeted tests:
