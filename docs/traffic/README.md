...
本目录是 fping/iPerf、本地执行、Windows Go Agent 执行以及 Traffic REST/WebSocket 契约的当前入口。历史 Web 迁移阶段不再单独维护。
...
```mermaid
flowchart TD
    WEB["Vue /tools/traffic (legacy /network-tools/traffic redirect)"] --> API["Traffic REST API / ws traffic"]
