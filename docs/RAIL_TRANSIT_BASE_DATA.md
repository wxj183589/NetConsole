...
“轨旁 AP 规划”标签页分为“AP 规划维护”和“AP 上线情况概览”。维护表固定为序号、车站名称、AP 数量、AP 管理 VLAN 和备注，支持直接编辑、Excel 整片粘贴、新增、删除、撤销、单元格校验和统一保存；车站必须匹配当前基础资料。AP 数量是当前确认应建设、应上线的有效规划值，可按现场核减调整；原设计数量和核减原因记入备注。同 VLAN 跨站合法，AC 状态、轨旁 AP 参考资料记录数和 VLAN 兼容模型均不覆盖规划值。概览表只展示规划 AP 总数量、实际上线、未上线、上线率和备注；实际上线按参考资料映射的稳定 `station_id` 汇总现有 FIT-AP 在线状态，未上线取 `max(规划值 - 实际上线, 0)`，总上线率取实际上线合计除以规划值合计。规划为 `0` 或实际上线超过规划时上线率显示 `—`，后者保留真实上线数量并显示“超规划”。未匹配有效资料的在线 AP 单独警告且不计入站点。“下载模板”生成可见 `AP规划`、`字段说明` 和隐藏 schema v3 元数据的 `轨旁AP逐站规划模板.xlsx`；“导出当前”读取已保存规划，生成 5 列 `AP规划` 与只读 `AP上线情况概览`，上线率使用百分比数值格式，合计按实际上线合计除以规划合计计算。导入只读取 `AP规划`，忽略说明和统计页；前 10 行可搜索表头，错误行保留在预览且不阻止其他有效行应用。旧工作表名“轨旁AP规划”、旧 8 列/22 列模板仍兼容，历史 IP、掩码、网关和组字段读取后忽略。规划预览不受基础资料锁定影响，但应用仍要求解锁。
...
- 轨旁 AP 规划导入只读取新 5 列模板或兼容旧 8/22 列模板；旧 IP、掩码、网关和组字段读取后忽略，不写入活动规划草稿。
