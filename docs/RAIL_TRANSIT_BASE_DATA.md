...
“轨旁 AP 规划”标签页分为“AP 规划维护”和“AP 上线情况概览”。维护表固定为序号、车站名称、规划 AP 总数量、AP 起始地址、掩码、AP 网关、AP 管理 VLAN 和备注，支持直接编辑、Excel 整片粘贴、新增、删除、撤销、单元格校验和统一保存。规划 AP 总数量是当前确认应建设、应上线的有效目标，可按现场核减调整；原设计数量和核减原因记入备注。同 VLAN 跨站合法，AC 状态、轨旁 AP 参考资料记录数和 VLAN 兼容模型均不覆盖规划值。概览表只展示规划 AP 总数量、实际上线、未上线、上线率和备注；实际上线按参考资料映射的稳定 `station_id` 汇总现有 FIT-AP 在线状态，未上线取 `max(规划值 - 实际上线, 0)`，总上线率取实际上线合计除以规划值合计。规划为 `0` 或实际上线超过规划时上线率显示 `—`，后者保留真实上线数量并显示“超规划”。未匹配有效资料的在线 AP 单独警告且不计入站点。“下载模板”生成仅含可见 `AP规划` 和隐藏 schema v2 元数据的 `轨旁AP逐站规划模板.xlsx`；“导出当前”读取已保存规划，生成 `AP规划` 与只读 `AP上线情况概览` 两个可见工作表，上线率使用百分比数值格式，合计按实际上线合计除以规划合计计算。导入只读取 `AP规划`，忽略统计页；旧工作表名“轨旁AP规划”、旧“AP数量”表头和旧 22 列 VLAN 分组模板仍兼容。规划预览不受基础资料锁定影响，但应用仍要求解锁。

轨旁 AP 业务页面、轨旁 AP 业务导出和上线概览共用当前局点的有效轨旁 AP 范围。范围按显式项目/线路/建设阶段、`work_scope_status=included`、有效 `station_id` 和稳定 AP 身份过滤并去重；暂不参与、明确排除、跨项目和未关联资源不进入正常统计。站名仅用于历史数据的唯一候选回退和最终展示，不作为聚合键；带序号与不带序号的同站名称不会生成两行。未配置建设阶段时不会按站名猜测一期或延长线。
...
2. 线路来源字段、站点名称/编码/来源键/路径顺序/折返类型、站点/区间引用、AP MAC、里程、MR 名称/IP/端口和规划 VLAN/站点归属校验通过；轨旁 AP 规划中的 IP 参考值不参与阻断；
