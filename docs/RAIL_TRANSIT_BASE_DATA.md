...
设备管理 `station` 字段开头的 1～3 位数字是辅助来源顺序，不属于正式站名；数字后可使用 `.`、`-`、`_`、`/`、顿号、逗号、冒号或空格，也可以不使用分隔符。`01小洋江站`、`01.小洋江站`、`01-小洋江站`、`1.小洋江站` 和 `小洋江站` 的规范站名都为 `小洋江站`，编号仅写入 `source_order_text/source_order`，普通车站再由 `source_order` 得到 `sort_order`。无数字前缀是合法来源，保留完整正式站名且 `sort_order=null`；只有数字、超过 3 位数字前缀或去除前缀后名称为空才阻断自动应用。

`source_station_value` 保留原始设备字段，`source_station_key/canonical_station_name` 保存规范站名身份；车辆段、停车场保留 `source_order`，但固定 `path_code=UNASSIGNED`、`sort_order=null`、`participates_in_direction=false`。来源匹配只使用“规范站名 + 节点类型”，编号不参与身份判断；同名不同节点类型、同一主线顺序对应不同规范站名、同一规范站名对应多个主线顺序均进入阻断冲突。

来源预览只给出候选、既有站点和“覆盖现有/新增/人工确认”建议，不提供直接写库接口。前端“从设备管理生成”和“导入模板”都只能应用到当前解锁草稿，最后仍通过全局 `validate` 与 `changes` 保存，继续保留 `base_revision`、明确确认、事务回滚和保存失败保留草稿。来源覆盖保留既有 `id/node_uid` 和所有引用，更新规范名称、编号、顺序、节点类型、路径与来源字段；中心里程、结构、站台、端点、折返、轨道设施、启用和备注等人工字段不会被来源同步覆盖。设备管理中来源后来消失时只把正式站点标为 `stale`，不删除站点，也不修改 `devices` 或 `device_groups`。
