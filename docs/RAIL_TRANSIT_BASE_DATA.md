...
轨旁 AP 业务页面、轨旁 AP 业务导出和上线概览共用当前局点的有效轨旁 AP 范围。范围按显式项目/线路/建设阶段、在用状态、有效 `station_id` 和稳定 AP 身份过滤并去重；暂停、退役、明确排除、跨项目和未关联资源不进入正常统计。站名仅用于历史数据的唯一候选回退和最终展示，不作为聚合键；带序号与不带序号的同站名称不会生成两行。未配置建设阶段时不会按站名猜测一期或延长线。
