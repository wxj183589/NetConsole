...
| Job Center | Qt Adapter + TaskRuntime + TaskRepository + worker/registry | 七状态、Event Hub、持久快照、REST/WebSocket 与 10 个 domain modules | 基础设施完成/领域部分迁移 | 任务中心是；领域逻辑否 | 宿主生命周期、`legacy_tasks.py` 兼容区 | 下一步接 Agent Adapter；legacy 只迁出 |
| FastAPI / Web Shell | Vue Dashboard/任务中心/Agent 管理、任务与 Agent API、OpenAPI、`--web-shell` | Application/API 与 Desktop Shell | 阶段 3 完成 | 任务基础设施与 Agent 控制面 | 无用户登录、Traffic 业务 API、正式 dist 发布打包 | 阶段 4B-2 接流量应用服务 |
...
| Go/CentOS/远程 Agent | Windows Go Agent + Python Agent Controller + Vue 管理页 | 配置、健康、版本、能力、fping/iPerf Typed Client、任务事件/结果 | 阶段 4B-1 协议完成 | Agent 资源管理与底层流量协议 | Controller 调度/任务映射、CentOS、主动注册、持久凭据 | 阶段 4B-2 接统一流量服务；CentOS 独立规划 |
...
阶段 4B-1 只新增 Agent fping/iPerf 强类型协议、任务事件/结果与 Python Typed Client，没有拆 `legacy_tasks.py`，也没有创建 Traffic 数据库、应用服务、Controller 调度、FastAPI Traffic 路由或 Vue 页面。上述未完成项仍是后续重构地图，不是已授权的实现计划。
