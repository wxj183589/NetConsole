...
| Online MR 实时采集 | Legacy Qt 页面通过兼容 Adapter 调用 LOCAL 生命周期，并可在独立对话框同步、匹配、下载和导入 Agent 已有包 | collection Application Service + Agent contract/typed download/import/controller + Qt Job 入口 | 5B-11 已接入 Qt Agent 包入口，远程执行仍未启用 | LOCAL 生命周期是；Agent 仅已有包同步/导入；Web 否 | 页面自管 Traffic 兼容方法、Agent start/stop/status 自动同步、FastAPI/Vue、离线解析接入 | 先真实验收 Qt 包同步/导入，再单独设计远程 start/status/stop；不改 LOCAL |
...
阶段 4C 已在阶段 4B-2 应用服务上增加 FastAPI Traffic 路由、按 Run 订阅的专用 WebSocket 和 Vue 流量测试页面，没有拆其他 `legacy_tasks.py`，也没有修改 Agent 协议。阶段 5B-3 至 5B-5 收口 Online MR LOCAL 生命周期并接入 Legacy Qt；阶段 5B-6 固化 Agent 契约，5B-7 增加安全 ZIP importer，5B-8 增加类型化查询、受控下载和 importer 编排，5B-9 增加维护脚本与 Controller 手工下载/导入门面，5B-10 复用既有 Agent Profile，增加只读包同步、局点静态设备 IP 候选和高层导入入口，5B-11 将该能力通过两个 Job 接入 Legacy Qt。当前仍未启用 AGENT executor、远程 start/status/stop、FastAPI/Vue 或离线解析接入，也没有修改 Go Agent、LOCAL 生命周期、设备/AC/FIT-AP/MESH、SNMP Center 或无线勘测。
