...
| 配置采集 | Config Application Service -> Router -> Vue | 两条勾选快照已进入真实对比输入；Monaco 只读主视图与结构化明细降级已接入，`IMPLEMENTED_UNVERIFIED` | 验证真实采集、裁剪、空/相同文件、Electron 离线 Worker、大配置和 Artifact |
