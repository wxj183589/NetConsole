...
截至 2026-07-14，NetConsole 已建立统一 Background Job 协议、独立 worker、进程适配器、Job Registry 和 11 个领域 handler 模块；Registry 当前注册 88 个任务类型。Traffic 的三个本地 handler 与 Online MR Agent 包同步/导入两个 handler 已直接调用正式 Service/Adapter，但多数既有领域 handler 仍通过 `legacy_handler(...)` 调用 `src/netconsole/services/job_center/handlers/legacy_tasks.py`，因此总体状态仍是“入口与协议统一，领域实现迁移中”，不是“已完成”。
...
| AC/FIT AP 资源 | Qt 页面提交 AC Job；Web 使用 GET-only Query Service | ac domain handlers/service + 只读 Web API | 5C-4 已增加总览、分页资源、Mesh Radio 1/2、LLDP、光衰与配置查看；5C-5 已增加 AC Mesh-Link 已落盘快照与 MR↔轨旁 AP 只读监控；采集仍在 Qt | 采集部分；Web 查询是 | 资源、历史等 legacy 适配；写操作仍 Qt | Web 保持只读，后续刷新必须走白名单 Task |
...
| FastAPI / Web Shell | Vue Dashboard/只读任务中心/Agent/AC/Traffic/Online MR 只读展示、对应 API、OpenAPI、`--web-shell`、普通主程序托盘 Web 控制台 | Application/API、Desktop WebHost 与临时本地会话 | 阶段 5C-5 已增加 GET-only AC Mesh-Link 快照、MR 状态与轨旁 AP 关联展示 | 任务、Agent、Traffic、AC 与 Online MR 查询 | Online MR Web 启停、AC 设备刷新/写操作、统一用户登录 | 继续从低风险只读页面迁移，不改变设备控制契约 |
...
阶段 5C-5 复用既有 AC Mesh-Link parser 和 `vehicle_mr_online.sqlite`，增加 GET-only 快照、MR 状态、轨旁 AP 匹配、过期判定和 Vue 在线监控。Web 不执行采集命令，旧快照没有 raw 时明确返回不可用，也不在轨道交通契约中引入客户端数量。
