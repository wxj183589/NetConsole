...
| Online MR 实时采集 | 页面会话 manager/worker；5B-1 只读 Query Service/DTO | collection start/status/finalize/package Application Service + 专用 runner | 5B-1 只读边界完成 | 查询边界是；启停否 | Qt Legacy 编排、多会话/外部进程生命周期 | 先实现已冻结的最终化契约，再接 API/UI；不改为一次性 Job |
...
阶段 4C 已在阶段 4B-2 应用服务上增加 FastAPI Traffic 路由、按 Run 订阅的专用 WebSocket 和 Vue 流量测试页面，没有拆其他 `legacy_tasks.py`，也没有修改 Agent 协议。阶段 5B-1 仅新增 Online MR DTO、纯 Python 只读 Query Service 与最终化文档契约；Qt Legacy 查询尚未切换，采集启停、Traffic/Agent 联动、FastAPI/Vue、设备/AC/FIT-AP/MESH、SNMP Center 和无线勘测均未迁移。
