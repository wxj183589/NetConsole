...
| Job Center | TaskRuntime + TaskRepository + worker/registry；Web 使用 Application/Query Service；Qt QProcess Adapter 已归位 `src/netconsole/ui/job_process_manager.py` | 七状态、Event Hub、持久快照、REST/WebSocket、统一任务窗口、外部 Agent Task 与 11 个 domain modules | Electron 任务窗口已接入；领域仍部分迁移，Qt Adapter 仅待删除 | 任务中心与 Web 查询是；领域逻辑部分 | `legacy_tasks.py` 兼容区、迁移期 Qt Adapter | legacy 只迁出；Qt 删除时移除 UI Adapter，不改永久协议 |
...
| Launcher / FastAPI / Desktop Shell | Electron Main/Preload + `netconsole.backend.electron_runtime`；`main.py --mode web|server` 仅开发诊断 | Electron 动态端口/临时令牌、唯一 Vue Renderer、显式冻结 Backend 分派 | E1 已删除 `netconsole.app`、Qt probe、`auto/qt`、旧 WebShell 与无调用 Qt Native Adapter；Electron 安全窗口和受管 Backend 已验证 | 启动与控制握手是；安装包/升级/托盘否 | Qt 页面与 Qt-only 测试仍待删除；无 Qt Backend 安装包尚未建立 | 继续 E1 清理 UI/测试/依赖，再建设 Electron-only 打包链 |
