...
| 统一 Traffic | `TrafficTestApplicationService` + Local/Agent Adapter + Supervisor | 本地/Agent iPerf/fping、任务映射、事件、运行索引 | 阶段 4B-2 完成 | 执行与同步是；Web 否 | 原 Qt iPerf/Ping 页面、无 Traffic API/Vue | 阶段 4C 只接 Web 入口 |
| FastAPI / Web Shell | Vue Dashboard/任务中心/Agent 管理、任务与 Agent API、OpenAPI、`--web-shell` | Application/API 与 Desktop Shell | 阶段 3 完成 | 任务基础设施与 Agent 控制面 | 无用户登录、Traffic 业务 API、正式 dist 发布打包 | 阶段 4C 接 Traffic REST/WebSocket/Vue |
...
| Go/CentOS/远程 Agent | Windows Go Agent + Python Agent Controller + Vue 管理页 | 配置、健康、能力、Typed Client、Traffic Adapter/Supervisor、任务事件/结果 | 阶段 4B-2 流量同步完成 | Agent 资源与 iPerf/fping 执行同步 | CentOS、主动注册、持久凭据、独立服务 | Web 入口阶段 4C；CentOS 独立规划 |
...
阶段 4B-2 只新增统一 Traffic 领域、本地/Agent 执行适配、运行索引、事件汇聚和远端同步，没有拆其他 `legacy_tasks.py`，也没有创建 FastAPI Traffic 路由、Traffic WebSocket 或 Vue 页面。Online MR、原 Qt iPerf/Ping、设备/AC/FIT-AP/MESH、SNMP Center、无线勘测均未迁移；阶段 4C 只能在现有应用服务上增加 Web 接入。
