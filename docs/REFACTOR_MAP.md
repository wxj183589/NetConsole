...
| 在线列车车地通信检测 | Web 按列车分别展示 MR-CT/MR-TC、Mesh-Link、Online MR、fping/iPerf、任务和包 | 只读聚合 Query Service + GET API + Vue | 5C-7A 已增加统一展示、受控 raw tail 和动态刷新 | 是，只读 | LOCAL 启停仍在 Qt；Agent 远程执行未开放；正式分析/报告未接入 | 保持展示边界，真实控制另行设计 |
