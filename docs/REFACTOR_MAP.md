...
| 已迁移（第三阶段） | `ui/pages/ac_management_page.py` | FIT-AP 资源、光衰和现有 AC 命令动作均已改为 Job + AC Domain；identity 阶段 2～4.1 只附加 shadow，页面流程未改 | 阶段 5 评估已完成；后续只做 MR/Mesh只读 shadow，不接管生产结果 |
...
2. AC 资源、光衰和命令动作 Domain Service 第一阶段已完成；AP identity 阶段 0～5 见 [AP_MODEL_ASSESSMENT.md](AP_MODEL_ASSESSMENT.md)、[AP_IDENTITY.md](AP_IDENTITY.md)、[TRACKSIDE_AP_IDENTITY_ASSESSMENT.md](TRACKSIDE_AP_IDENTITY_ASSESSMENT.md) 与 [MR_MESH_AP_IDENTITY_ASSESSMENT.md](MR_MESH_AP_IDENTITY_ASSESSMENT.md)，下一阶段只做 MR/Mesh只读 shadow。
...
- 推荐路线固定为：identity 工具（已完成）→ AC/extension shadow（已完成）→ 光衰 shadow（已完成）→ 轨旁只读评估（已完成）→ 轨旁只读 shadow（已完成）→ MR/Mesh shadow 评估（已完成）→ MR/Mesh只读 shadow → 导出去重。
