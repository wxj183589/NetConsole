...
- 当前 Web 页面包含 Dashboard、只读任务中心、Agent 管理、AC 管理、Traffic 和只读 Online MR 实时展示；任务中心通过 GET-only `/api/job-center` 查询任务快照、结构化事件和 Online MR 映射，不提供 stop、force-stop、delete 或 retry；
