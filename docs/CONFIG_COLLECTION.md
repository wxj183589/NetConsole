...
`config_compare_snapshot_pair` Job 按快照 ID 在当前数据根内解析安全文件引用，读取文本并返回 `left_text`、`right_text`、`diff_rows`、`diff_summary`、`raw_diff` 和左右标签。页面默认使用只读 Monaco Diff Editor 展示后台返回的左右完整文本；“差异明细”保留左行号、左文本、状态、右行号和右文本，并支持全部/新增/删除/修改过滤。上一处、下一处仍由完整 `diff_rows` 生成目标，不使用 Monaco 内部命令作为业务事实。
...
- Monaco 初始化或 Worker 加载失败时自动切到结构化差异明细，保留后台摘要、导航和 Artifact。左右文本合计超过 4,000,000 字符或 100,000 行时也明确提示并切到明细，不截断内容。
- `diff_summary`、`diff_rows`、`raw_diff` 和 Artifact 继续以 Python Job 结果为准；Monaco 自身的渲染差异不回写业务摘要，也不参与配置合规、保存、合并或下发。
