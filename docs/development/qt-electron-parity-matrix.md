...
| 轨道交通 | 轨道交通及其子页 | `RailTransitPage`、`CarNetworkDiagnosticPage`、`OnlineMrCollectionPage`、`MeshLogAnalysisPage` 等 | `/rail-transit/*` / `views/rail-transit/` | 基础资料、在线列车、车地通信、车内检测、Online MR、Mesh 分析和报告 | Rail/Online MR/Mesh Application Service；相关 Router | 多个真实纵向闭环已接入；公共 handler/Artifact、Qt 剩余按钮和真实设备验收仍未完成，详见[轨交逐操作矩阵](parity/rail-transit.md) | `PARTIAL` |
...
| 轨道交通 | 独立业务入口已拆分，仍有交互缺口 | 多个真实 Application Service/Task/Export 闭环，仍有公共依赖 | 基础资料/MESH/Online MR 已有，点表/在线列车/轨旁仍缺 | 已有取消、恢复和 LOCAL 强停；部分 Job 待公共注册 | Rail/Online MR/Mesh 定向测试，详见[轨交逐操作矩阵](parity/rail-transit.md) | `NOT_STARTED` | `REAL_DEVICE_PENDING`，但功能缺口未清零 | 公共 handler/Artifact、剩余 Qt 按钮、人工对照；不得以聚合看板替代 Qt 业务页 |
