...
本文按当前 Qt 源码、Vue 路由、FastAPI Router、Application Service 和测试记录判断迁移状态，不以“页面已经存在”作为完成依据。审计基线为 `main@3d3b39f0`，设备管理专项实现位于 `codex/qt-electron-device-parity`。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 设备管理 | 已实现，待逐项对照 | 真实 CRUD/分组/连接/采集/终端链路已实现 | CSV 导入预览与确认、CSV/模板/SecureCRT/OmniPeek 实际 Artifact | Job Center 任务、轮询、取消和 sessionStorage 恢复 | `tests/test_device_management_web_api.py`、`tests/test_desktop_action_service.py`、`DeviceManagementView.test.ts` | `NOT_STARTED` | SSH/Telnet/SNMP、采集、光模块、诊断下载均待现场设备 | 人工 Qt/Electron 同数据逐项对照；真实设备结果与部分失败语义 |
...
3. 当前设备管理只能标记 `IMPLEMENTED_UNVERIFIED`。人工对照通过但现场设备未通过时，升级为 `REAL_DEVICE_PENDING`；全部通过后才能升级为 `COMPLETE`。
