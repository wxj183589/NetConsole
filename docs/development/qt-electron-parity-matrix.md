...
| AC 管理 | FIT-AP 主页面已接打开 AC Web、AC 信息、普通资源、AP 详情深度、光衰更新、批量删除和元数据导入；受控页已接 Qt 两项 AC 写操作 | HTTPS 外部打开、四类采集、批量删除、CSV/XLSX 元数据导入与两项固定写操作均为真实闭环，无 AC Fake 执行 | 部分 | 进度、取消、失败、部分失败、重启恢复和动作审计已接入 | AC parser/repository/job/API/Vue 定向测试 | `NOT_STARTED` | `REAL_DEVICE_PENDING` | AP/光衰/OmniPeek 导出受共享 Artifact 契约阻塞；扩展/规划完整编辑导入导出、其余 Qt 子页和现场验收 |
