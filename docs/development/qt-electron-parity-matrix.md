...
| AC 管理 | FIT-AP 主页面已接真实更新任务，其他子页部分 | FIT-AP 普通更新真实；AC 信息/单 AP 深度/动作仍待迁移 | 部分 | FIT-AP 进度、取消、失败、部分失败、重启恢复已接入 | AC parser/repository/job/API/Vue 定向测试 | `NOT_STARTED` | `REAL_DEVICE_PENDING` | AC 信息、光衰主入口、单 AP 深度、真实写操作和现场验收 |
