...
| 命令说明 | 命令说明 | `CommandReferencePage` | `/command-reference` / `CommandReferenceView.vue` | 搜索、模块/设备/厂商/协议/类别/风险筛选、详情、复制、刷新、Markdown 导出 | `CommandReferenceApplicationService`；`command_reference_router.py`；查询链已接通，导出复用 Export Process 与统一 Artifact 契约 | 查询与页面已实现；生产 Artifact 来源注册等待统一任务窗口，当前为 `BLOCKED_ON_TASK_WINDOW` | `PARTIAL` |
...
| 网络工具 | 核心页面齐全 | Network/Traffic/Wireless 真实 Service | CSV/XLSX Artifact 已有 | 共享 Task 状态、停止、恢复和 Traffic WebSocket | Network/Traffic API、Service、Job、Artifact 与 Vue 定向测试 | `NOT_STARTED` | Agent/iPerf/无线硬件待验收 | IPOP 依赖系统设置与语义 Native Bridge；未达到 `REPLACE_READY` |
...
| 命令说明 | 已实现，待人工对照 | 查询、筛选、详情、复制与刷新已实现 | Markdown ExportJob 已实现；生产 Artifact 来源注册被统一任务窗口阻塞 | 持久 Task、轮询、取消与 localStorage 恢复已实现 | `tests/test_command_reference_web_api.py`、`tests/test_command_reference.py`、`CommandReferenceView.test.ts` | `NOT_STARTED` | 不适用 | `BLOCKED_ON_TASK_WINDOW`：公共 `WebArtifactStore` 尚未登记 `command_reference_export` 来源 |
