...
| 配置采集中心 | 配置采集中心 | `ConfigCollectionCenterPage`、`ConfigLifecycleWorker/Service`、`ConfigDiffViewer`；详见 [配置采集对等矩阵](parity/config-collection.md) | `/config-center` / `ConfigCollectionView.vue` | running/saved/diff 真实采集、`save force`、批量、历史/内容/删除、同/跨设备双栏比较、差异导航、导出/Artifact、取消/失败/恢复 | `ConfigCollectionApplicationService`；Config Router；config domain handlers | 真实纵向链与自动化已接入；共享任务窗口取消 capability 需集成补丁，真实 H3C 设备待验收 | `REAL_DEVICE_PENDING` |
...
| 配置采集中心 | 双栏与操作入口已实现 | 真实采集/保存/历史/删除/比较 | Export Process + Artifact 已实现 | Task 持久化、检查点取消/失败/恢复已实现 | Config Service/API/Qt diff/Vitest/build 定向验证 | 主工作树 Electron 点击待验收 | H3C SSH/Telnet 与现场权限待验收 | 共享任务窗口需放开配置不可逆任务取消 capability；不得标 `COMPLETE` |
