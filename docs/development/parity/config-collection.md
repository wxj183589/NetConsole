...
当前导航状态为 `IMPLEMENTED_UNVERIFIED`，不是 `REAL_DEVICE_PENDING` 或 `COMPLETE`。自动化已覆盖不连接真实设备的服务、API、Vue 真实 mount 交互、类型构建、统一任务窗口取消、检查点、失败与重启恢复；H3C SSH/Telnet、现场权限、设备返回差异及 Electron 人工点击仍需主工作树验收。Qt 页面继续保留为事实源和回退入口。
...
| 单设备配置采集 | 固定执行 `screen-length disable`、`display current-configuration`、`display saved-configuration` | Vue 提交持久 Task → config handler → `ConfigLifecycleService.fetch_configs` → running/saved/diff 三类快照和共享 raw/jsonl 审计 | `tests/test_config_lifecycle_service.py`、`tests/test_config_collection_web_api.py`、`ConfigCollectionView.test.ts` | 真实设备待验收；模块仍为 `IMPLEMENTED_UNVERIFIED` |
...
| 批量保存配置 | 逐设备执行、保留失败项 | 单一持久 Task，逐设备检查点；可从统一任务窗口请求取消，强停时当前项为 unknown、剩余项为 not_started，重启恢复结构化结果 | `tests/test_config_collection_web_api.py` | 真实设备待验收 |
...
| 删除一个或多个快照 | 用户确认后删除记录和文件 | 服务端一次性确认 → 持久 Task → Service/Repository；受控快照及最后引用的 raw/jsonl 先同目录原子隔离，DB 失败逆序恢复，DB 成功后清理；清理受 ACL/杀软阻挡时只保留受控 `.pending` 文件 | `tests/test_config_lifecycle_service.py`、`tests/test_config_collection_web_api.py`、`ConfigCollectionView.test.ts` | 无 schema 变更 |
...
| 任意两快照比较 | 支持同设备、跨设备和不同历史时间 | 快照表提供独立左右选择篮；切换设备或配置类型不清空已选侧；API、页面和导出头统一使用“设备名 · 类型 · 时间” | `tests/test_config_collection_web_api.py`、`ConfigCollectionView.test.ts` | 无 |
...
| 差异筛选与导航 | Qt 可浏览完整双栏结果 | Vue 支持全部/新增/删除/修改筛选、上一处/下一处滚动定位；真实 mount 测试由 API mock 驱动跨设备左右选择与下一处导航；API 同时接受 added/removed/modified 过滤 | `tests/test_config_collection_web_api.py`、`ConfigCollectionView.test.ts` | 无 |
...
| 任务进度、日志、取消、失败和重启恢复 | Qt Worker/Job 展示进度和失败 | 配置主页只显示运行/异常计数并打开独立共享任务窗口；TaskRepository 持久化，检查点恢复不可逆批次，Artifact 由共享任务窗口交付 | `tests/test_config_collection_web_api.py` | Electron 人工窗口验收待完成 |
| 配置导入 | Qt 配置采集中心没有配置导入入口 | 不新增 Electron 配置导入入口 | 不适用 | 保持 1:1，不以新设计扩范围 |

## 共享任务窗口边界

累计集成分支消费唯一 `window.netconsoleDesktop.openTaskWindow({ module: "config" })`，并由真实 mount 和 API 测试验证入口与取消路由。配置 Task 在 `PENDING/STARTING/RUNNING` 且本地 owner 接收端存活时可请求取消；`STOPPING` 明确显示已请求停止。配置 Worker 在每个设备/快照边界调用 `check_cancelled()`，并用检查点记录 completed/current/pending，强停或重启可交付 unknown/not_started 结果。未创建第二套 Task、Artifact 或 Native Bridge。
