...
当前导航状态为 `IMPLEMENTED_UNVERIFIED`，不是 `REAL_DEVICE_PENDING` 或 `COMPLETE`。自动化已覆盖不连接真实设备的服务、API、Vue 真实 mount 交互、类型构建、取消检查点、失败与重启恢复；共享任务窗口的公共取消 capability 尚未由 task 分支集成，H3C SSH/Telnet、现场权限、设备返回差异及 Electron 人工点击仍需主工作树验收。Qt 页面继续保留为事实源和回退入口。
...
| 删除一个或多个快照 | 用户确认后删除记录和文件 | 服务端一次性确认 → 持久 Task → Service/Repository；受控快照及最后引用的 raw/jsonl 先同目录原子隔离，DB 失败逆序恢复，DB 成功后清理；清理受 ACL/杀软阻挡时只保留受控 `.pending` 文件 | `tests/test_config_lifecycle_service.py`、`tests/test_config_collection_web_api.py`、`ConfigCollectionView.test.ts` | 共享任务窗口停止能力依赖见下节 |
...
本分支只消费 `window.netconsoleDesktop.openTaskWindow({ module: "config" })`，并由真实 mount 测试验证入口调用；未修改 Job Center、TaskRepository、TaskApplicationService、公共 Artifact 或 Native Bridge。公共取消能力继续由 task 分支单写，本模块只保留 owner cancel receiver 和逐项目检查点。共享分支集成时仍需满足：
...
未合入该共享能力前，配置 API 的取消与恢复契约已可用，但统一任务窗口按钮仍会被 capability 查询禁用，因此状态保持 `IMPLEMENTED_UNVERIFIED`，不得标记为 `REAL_DEVICE_PENDING` 或 `COMPLETE`。
