...
- `apps/desktop_renderer` 只放 Vue/TypeScript/Vite 源码、前端配置和锁文件；`node_modules`、前端 `dist` 和 TypeScript 缓存不得提交。
...
- 自动测试只能使用显式 `<repository-parent>\test-data\NetConsole\<run-id>`，不依赖安装目录或当前工作目录。
...
- 测试生成的数据库、日志、报告、缓存和临时文件必须位于 `<repository-parent>\test-data\NetConsole\<run-id>`，不能写回源码树或真实数据根。
