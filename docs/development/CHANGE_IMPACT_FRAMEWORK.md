...
Gate 强制使用 `RuntimeMode.TEST` 与唯一 `<repository-parent>/test-data/NetConsole/<run-id>`，报告写入 `.local-reports/`；本地与 GitHub Actions 共享同一解析规则。必要的真实 `D:\NetConsoleData-dev` 验收和 Release Gate 仍需单独执行，也不得把未执行的 GUI、设备、安装包或长时运行验收写成通过。
