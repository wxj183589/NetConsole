...
自动测试必须显式设置 `RuntimeMode.TEST` 和 `NETCONSOLE_DATA_ROOT=<repository-parent>/test-data/NetConsole/<run-id>`。测试根不存在、直接指向 `<repository-parent>/test-data/NetConsole`、或位于该根以外都会失败。测试清理仅限自己的 `run-id`，不得接触 `D:\NetConsoleData`。
