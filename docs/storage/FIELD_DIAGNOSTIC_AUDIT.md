...
- Windows CIM/Performance Counter 快照、常见 RAID CLI 存在性检测。
- metadata-only 文件库存、最大文件、推断活动写文件。
- 有界日志 tail 和 Raw NDJSON 样本。
- `manifest.json`、`summary.json`、`bundle_integrity.json` 及 ZIP 原子生成。

诊断 Worker 不连接业务数据库写事务，不执行 checkpoint、VACUUM、repair、清理或服务停止。运行态摘要通过任务参数传递；未提供时以 `unavailable/null` 表示，不伪造零值。
