...
启动不根据目录名猜测环境。持久化根必须包含有效的 `runtime_mode.json`；缺失、损坏、`test` 标记或生产根关闭只读警告时拒绝启动。测试进程仍使用显式 `RuntimeMode.TEST` 和 `<repository-parent>\test-data\NetConsole\<run-id>`，不写持久化 marker。
