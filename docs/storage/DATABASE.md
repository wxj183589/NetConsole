...
The central MESH `catalog.sqlite` now declares `mesh_catalog_v3_source_lifecycle`. Its domain migration owns the ordered `v1 device UUID -> v2 issue severity -> v3 source lifecycle` steps and runs the version check, all additive DDL, lifecycle table creation, final version write, and schema validation under one `BEGIN IMMEDIATE` transaction. `MeshCatalogRepository` no longer contains per-column `PRAGMA table_info -> ALTER TABLE` branches, so concurrent delete/import workers cannot both decide that `info_count` is absent. New catalogs are born with the complete latest schema. This catalog migration is currently domain-owned and transactional; it is not yet a shadow-copy `DatabaseUpgradeCoordinator` adapter, so the shared backup-center claim remains limited to `mesh_derived`.

The Profile `mesh.sqlite` and source-index compatibility readers still support historical compact databases. Their legacy additive checks now acquire `BEGIN IMMEDIATE` before reading columns, serializing a remaining `PRAGMA/ALTER` compatibility path across processes. This removes the TOCTOU race but is not the final architecture: a future compact schema bump must move those remaining additions into an explicit `mesh_derived` migration/rebuild and leave Repository construction read-only for existing databases. Tests and release notes must not describe those compatibility checks as shared upgrade-framework adoption.

Online MR `parsed/online_diagnosis.sqlite` is a per-Session derived database with a domain-owned automatic ensure path; it is not yet a `DatabaseUpgradeCoordinator` adapter. The Online MR service compares declared capabilities and real tables, rebuilds only from that Session's preserved raw artifacts into `online_diagnosis.sqlite.upgrading`, validates integrity, raw fingerprint, metadata, row counts and business projections, then atomically publishes. The previous active database remains readable until publication and one verified rollback copy is retained at `parsed/retired/online_diagnosis.previous.sqlite`; repeated successful rebuilds replace that single rollback slot instead of accumulating unbounded Session-local copies. Missing or unchanged failed raw input is recorded and does not repeatedly create Jobs. This scoped lifecycle must not be described as global backup-center adoption; moving it into the shared coordinator requires a dedicated Online MR adapter and retention migration.
...
are not rewritten. See [Trackside AP station planning](../rail-transit/base-data/TRACKSIDE_AP_PLANNING.md).
