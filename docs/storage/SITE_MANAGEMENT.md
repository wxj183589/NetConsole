...
预检通过后，当前 Renderer 发送 `before-site-switch`，保存当前进程内可回滚工作区快照并移除 Dashboard、系统设置之外的局点业务标签；MESH 在该阶段中止详情/轨旁请求并释放 ECharts 与轨旁 series cache。激活接口更新现有应用配置并返回 `restart_required=true`；Electron 停止旧 Backend 后、启动新 Backend 前收敛所有工作区窗口的内存快照。新 Backend 只有在 health ready 且 `/api/v1/sites/active` 与目标局点一致后才被接受，随后清除旧 Renderer 恢复状态并刷新所有 Renderer；新进程重新加载 Feature Gate、导航和当前局点。任一步失败时 Electron 恢复原 Backend 和其他窗口快照，发起窗口恢复本地快照并保持可操作。托盘“快速切换局点”只向 Renderer 发送目标 `site_id` 意图，Renderer 回到设置页共享这条流程；Electron Main 从 Backend `/api/v1/sites/active` 和 `/api/v1/sites` 重新读取 `display_name` 与清单来更新托盘，不接收 Renderer 提供的名称或清单。该流程只清理内存状态、标签和 query，不删除、移动或改写旧局点数据库、raw、报告与 Artifact。
