...
| Tasks | `tasks.db`, active/current-recent state, operational event tail, immutable result references and recovery | `task_result_blobs` is the current terminal-result body authority; `TaskHistoryStore` is retained only as compatibility/maintenance evidence and is not a user-facing archive or runtime authority | `TaskRepository`, `TaskResultBlobRepository`, `TaskCleanupService` |
