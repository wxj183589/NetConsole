...
| AC 管理 | FIT-AP 扩展信息 | 查看、导入预览、受控写入 | `ac.fit_ap_extensions` | AC 管理 | `/ac-management/extensions` | `web.ac_extensions` | 现有 FIT-AP 扩展 Service | `NOT_STARTED` | 是 | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | 正式写入需预览、确认、Task 和审计。 |
...
| AC 管理 | AC 受控动作 | 信息刷新、固化新 AP、`save force`、开启 AP 远程登入 | `ac.ac_info_update`、`ac.ac_actions` | AC 管理 | 规划路由内动作 | 待登记动作 Feature | AC Application Service、Task、审计 | `NOT_STARTED` | 是 | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | 禁止任意命令输入和 Vue 拼命令。 |
...
| 轨道交通 | 车内通信检测 | 检测、进度、结果与导出 | `rail.car_network_diagnostic` | 轨道交通 | `/rail-transit/car-network-diagnostic` | `web.rail_car_network_diagnostic` | Car Network Diagnostic Service、Job/Export | `NOT_STARTED` | 是 | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | 与在线列车车地通信检测是不同业务。 |
...
| 轨道交通 | MR 原始 MESH 日志分析 | 文件选择、导入、重建、分析、取消、报告 | `rail.raw_mesh_log_analysis`、`mesh.generate_report` | 轨道交通 | `/rail-transit/mesh-analysis` | 待登记动作 Feature | 正式 Mesh parser、Job Center、Export Process | `NOT_STARTED` | 是 | `NOT_STARTED` | `NOT_STARTED` | `NOT_STARTED` | 现有只读页不能视为完整替代 Qt 分析工作流。 |
...
- 已有 Web 页面解决了部分只读展示、受控 Task 和 Fake 闭环，但设备正式 CRUD、AC 受控写、轨交缺失 Tab、设备远程文件、无线扫描及系统类页面仍未完成。
