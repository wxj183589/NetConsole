...
本矩阵用于 v1.5.6 跨电脑交付门禁。`release included=是` 表示功能代码、页面和 API 属于正式构建契约，不代表已经完成真实 Windows/真实设备人工验收。自动测试不能替代 NSIS 安装、跨电脑导入或现场 SSH。本文保持 v1.5.6 版本隔离和原始人工验收状态；当前 v1.5.8 及主线能力状态见[真实设备能力矩阵](../development/REAL_DEVICE_CAPABILITY_MATRIX_20260920.md)和 [CHANGELOG](../CHANGELOG.md)，不得把其他版本制品的验收结果回填到本表。入口实现与五类验收证据的交叉视图见[迁移矩阵事实维度验收矩阵](../archive/migrations/qt-to-electron/MIGRATION_MATRIX.md#事实维度验收矩阵)。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `module.devices` | 设备管理 | `ENABLED` | 是 | 是 | 否 | 是 | 可见、可查询 | `/api/device-management/devices` | `/devices` | `test_device_management_web_api.py`、`DeviceManagementView*.test.ts` | `PENDING` |
| `capability.devices.connection_test` | 已保存设备连接测试 | `ENABLED` | 是 | 是 | 否 | 是 | 前置校验后可创建任务 | `/api/device-management/devices/{id}/connection-tests` | `/devices` | `test_device_management_web_api.py` | `PENDING` |
| `capability.devices.form_connection_test` | 未保存表单连接测试 | `ENABLED` | 是 | 是 | 否 | 是 | 一次性凭据、任务参数无秘密 | `/api/device-management/devices/test-connection` | `/devices` 编辑弹窗 | `test_device_management_web_api.py`、`DeviceManagementView.test.ts` | `PENDING` |
| `capability.devices.collect` | 设备采集与诊断 | `ENABLED` | 是 | 是 | 否 | 是 | 可提交采集/诊断 | `/api/device-management/devices/*/collect` | `/devices` | `test_device_management_web_api.py`、`test_device_detail_web_api.py` | `PENDING` |
| `capability.devices.import` | 设备导入 | `ENABLED` | 是 | 是 | 否 | 是 | 可预览并受控导入 | `/api/device-management/imports/*` | `/devices` | `test_device_management_web_api.py` | `PENDING` |
| `capability.devices.export` | 设备导出 | `ENABLED` | 是 | 是 | 否 | 是 | 进入 Export Process | `/api/device-management/exports/*` | `/devices` | `test_device_management_web_api.py` | `PENDING` |
...
| --- | --- | --- | --- |
| `module.feature_switch`、`system.feature_flags`、`internal.feature_switch` | `internal_only` | 不显示、不启用、配置 API 受控拒绝 | FeatureGate pytest、设置 API pytest |
| `capability.ac.online_overview`、`capability.ac.extensions*`、`capability.ac.config_snapshots` | `DEVELOPMENT` | 不显示、不启用 | FeatureGate pytest、构建 Gate |
| `capability.online_mr.local_control`、`capability.online_mr.agent_control` | `DEVELOPMENT`，未接入生产调用 | 不显示、不启用 | FeatureGate pytest |
| `module.train_communication` | 隐藏兼容、`DEVELOPMENT` | 不注册正式入口 | FeatureGate/导航测试 |
