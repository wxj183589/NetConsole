...
安装器数据根探测还必须覆盖：空目录、空根在注册表发布前生成可解析且根一致的 manifest、损坏 manifest 不覆盖、含普通文件目录、含 manifest/SQLite/采集文件的既有根、D 盘目标、C 盘安装器临时目录、不可写目录、旧固定探测残留、唯一名称冲突、失败清理、中文路径、尾随反斜杠和重复安装。验证需比较探测前后既有业务文件的内容哈希、大小和修改时间，并扫描 `.netconsole-install-probe-*` 残留；禁止用删除、改名或迁移真实数据根换取通过。
