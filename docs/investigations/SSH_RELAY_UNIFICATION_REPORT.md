...
The package must be generated by the existing Windows Production pipeline only,
after the final source commit is pushed. Fill the following values from the
pipeline artifact manifest; do not infer them from an unpacked build directory:
...
of AC/Switch/FIT-AP/Optical/LLDP real-network acceptance.
