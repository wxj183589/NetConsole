...
本文是 NetConsole 新增或修改用户文件交互的永久开发契约。2026-07-30 已有页面的扫描结果、整改证据和人工验收状态见 [UI 导入导出文件选择审计](development/import-export-dialog-audit.md)；后续实现不得把该审计矩阵当作唯一规则来源。
...
5. **测试和构建输出**：自动化测试、打包和 smoke 流程产生的非用户文件。
...
4. **内部 Artifact、任务日志、缓存和临时文件**：不属于用户最终副本，不弹 Save As。
...
Export Process 的内部 Artifact、临时文件、JSONL、取消和原子替换规则见 [导出进程规范](export_process_policy.md)。
