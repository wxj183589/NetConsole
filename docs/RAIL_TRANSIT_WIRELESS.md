...
轨旁查询、筛选项、统计、导出和自动任务目标共用“当前项目有效轨旁 AP 范围”：交换机设备必须是 `work_scope_status=included`，并在当前局点/项目配置了建设阶段时匹配该阶段；AP 资料还必须关联有效 `station_id` 和稳定 AP 身份，并按 UUID/MAC/受控唯一名称去重。暂不参与、明确排除、跨项目、阶段不匹配和未匹配在线资源进入排除项；未配置建设阶段时不按站名猜测一期或延长线。Worker 在连接前再次检查当前工作状态，期间转为 `excluded` 时按“当前工作状态为暂不参与”跳过；用户在设备管理中明确发起的手动只读操作仍允许执行。同一局点、同一更新范围的活动任务复用原 Task ID，取消任务会关闭当前设备会话。完整边界和后续清单见 [ZTE 轨旁交换机 Adapter](ZTE_TRACKSIDE_SWITCH_ADAPTER.md)。
