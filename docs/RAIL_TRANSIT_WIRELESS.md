...
轨旁 AP 业务使用独立维护规则：交换机侧和 AP 侧接收光功率低于 `-13 dBm` 时标记“功率异常”，恰好 `-13 dBm` 仍为正常边界。该规则只作用于轨旁查询、筛选、统计和导出，不修改设备详情或 ZTE 厂商原生 RX/TX 门限；无光、无模块、离线、链路异常及更严重的原生告警继续优先保留。历史快照也在查询和导出时按该业务规则重新计算，避免旧的“正常”状态继续显示。
