...
轨旁 AP 页面和业务导出已使用固定 revision 只读快照：页面携带 expected revision，导出提交时冻结筛选/选择后的基础业务行和既有工作簿输入，Worker 不重新查询 FIT-AP、LLDP、基础资料、Identity 或光衰数据库。来源持续变化时有限重试，过期、无效或缺失快照返回具名错误，不以长时间资源锁阻塞采集。完整数据源和 Artifact 边界见 [轨旁 AP 业务只读快照](TRACKSIDE_AP_BUSINESS_SNAPSHOT.md)。
