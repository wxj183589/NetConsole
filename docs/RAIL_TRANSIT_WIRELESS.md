...
轨旁 AP 业务使用独立维护规则：交换机侧和 AP 侧接收光功率严格低于 `-13.90 dBm` 时标记“功率异常”，恰好 `-13.90 dBm` 仍为正常边界。该规则只作用于轨旁查询、筛选、统计和导出，不修改设备详情或 ZTE 厂商原生 RX/TX 门限；无光、无模块、离线及更严重的原生告警继续优先保留。端口 `DOWN` 在综合状态显示“链路断开”，模块列保留模块自身状态；历史快照不参与本轮实时状态判断。
