...
轨旁查询、筛选项、统计、导出和任务目标共用“当前项目有效轨旁 AP 范围”：只纳入当前局点/项目、显式建设阶段匹配、投运状态为在用、已关联有效 `station_id` 且具有稳定 AP 身份的资料，并按 UUID/MAC/受控唯一名称去重。暂停、停用、退役、明确排除、跨项目和未匹配在线资源进入排除项；未配置建设阶段时不按站名猜测一期或延长线。Worker 在连接前再次检查交换机设备状态，期间转为非在用时按跳过记录；同一局点、同一更新范围的活动任务复用原 Task ID，取消任务会关闭当前设备会话。完整边界和后续清单见 [ZTE 轨旁交换机 Adapter](ZTE_TRACKSIDE_SWITCH_ADAPTER.md)。
