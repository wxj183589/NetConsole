...
- Running `main.py` without arguments starts the source Electron Desktop. `--mode server` is local development diagnostics only, not a second product runtime.
...
- Business code lives in `src/netconsole`; Electron, Web, and Agent live in `apps/desktop_electron`, `apps/desktop_renderer`, and `apps/agent`. Do not recreate historical or vague directories such as `apps/desktop`, `src/netconsole/ui`, `frontend`, `desktop`, `netconsole`, or `project`.
...
- Tool Collection is an Electron Desktop-only launcher for user-selected third-party `.exe` files. It uses `module.tools` / `module.tools` and independent Electron `userData` storage; it must not write into the Python Backend, site databases, the data root, `.ncsite`, or `.ncresult`, and launch/reveal requests pass only tool IDs.
