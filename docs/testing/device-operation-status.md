...
& 'D:\study\NetConsole\.venv\Scripts\python.exe' -m pytest `
  tests/test_device_operation_status.py `
...
1. 启动 Electron，打开设备管理，确认默认投运状态为“在用”，迁移前设备仍显示。
2. 新建测试设备，确认默认建设阶段“未指定”、投运状态“在用”。
3. 手工选择测试用二期设备，批量设置“二期 + 未并网”并填写原因。不要按名称编号或 IP 范围自动选择。
4. 返回默认列表，确认该设备不显示；切换“全部”及“二期 + 未并网”，确认设备、原因和更新时间仍可见。
5. 对未并网设备执行连接测试、详情刷新、配置采集和外部终端，确认手动任务可提交。
6. 检查无人值守与 AC 自动候选，确认未并网、调试中、暂停使用和已退役设备不会自动入选。
7. 将测试设备改为已退役，确认历史可查看和导出，新采集被拒绝。
8. 将测试设备改回在用，确认无需重启即可恢复默认显示和自动候选。
9. 导出 CSV，确认包含三个生命周期列；用上一版模板更新设备，确认状态不变；新模板空状态单元格更新时也保留原值。
