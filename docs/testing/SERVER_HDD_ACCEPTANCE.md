...
> [DATA_LAYOUT.md](../storage/DATA_LAYOUT.md)
> 为准。
...
本文的旧 Phase 2.1 验证命令已随 Legacy external HistoryStore maintenance 在 2026-08-30
退役；保留本节仅作为当时验收事实，不再提供可执行入口。
...
6. 在 `SERVER_UNATTENDED ACTIVE` 下运行 Syslog、MR、长 Ping/Traffic，确认任务 persistence 优先；不启动 migration/retention/VACUUM。
