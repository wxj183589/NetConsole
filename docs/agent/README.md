...
当前构建和开发验收目标为 Windows 10、Windows 11 和 Windows Server 2016 x64。Windows Server 2012/2012 R2 不属于后续开发、测试或发布范围；不新增按操作系统阻断 Agent 启动的逻辑。CentOS 7.4 没有 Agent 实现或离线交付包。离线 Windows 构建只使用仓库锁定依赖、Go module 缓存和 `resources/tools/windows-x64/` 白名单资源，不在构建时联网补齐业务工具。
