...
本矩阵用于 v1.4.2 跨电脑交付门禁。`release included=是` 表示功能代码、页面和 API 属于正式构建契约，不代表已经完成真实 Windows/真实设备人工验收。自动测试不能替代 NSIS 安装、跨电脑导入或现场 SSH。
...
局点管理当前是 `web.system_settings` 内的正式面板，不建立第二个 Feature ID。其 API 位于 `/api/sites` 与局点包相关路由；`.ncsite` 清洗、导入和 `needs_reentry` 闭环由 `test_site_storage.py / test_site_storage_api.py` 验证。
...
| 跨电脑导入普通 `.ncsite` | 包清洗与 `needs_reentry` pytest | `PENDING` |
| 重新录入凭据后真实/仿真 H3C SSH | Repository/连接任务自动闭环 | `PENDING` |
