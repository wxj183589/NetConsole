...
| 统计口径 | 数量 | 说明 |
| --- | ---: | --- |
| 交换机命令 | 29 | 设备详情、接口、光模块、LLDP、配置采集、诊断、文件管理、CLI ping、SFTP 准备等 |
| 无线 / MR / Mesh 命令 | 31 | AC、FIT-AP、MR 实时采集、Mesh 链路、ar5drv、repeat 采集等 |
| 本地工具 / 连接协议 | 16 | ping、fping、iperf3、netsh、PowerShell、WinSCP、外部终端、SSH/Telnet、SFTP/SCP |
| SNMP / RESTful 接口说明 | 7 | SNMP Get/GetNext/GetBulk/Walk/BulkWalk/TableWalk/Set/Trap 和 HTTPS/RESTful 现状 |
...
| SNMP 中心 | SNMP 中心 | 网络设备 | SNMP | `snmp_query_history`、`snmp_set_history`、`snmp_traps` | `snmp_client.py`、SNMP 页面 |
...
SNMP / RESTful 不归入设备 CLI 命令表，统一在 JSON 中以 `is_cli=false` 标记。

| 接口 | 用途 | 关键 OID / MIB | 风险 |
| --- | --- | --- | --- |
| SNMP Get | 查询单个实例 OID | `sysName`、`sysDescr`、`sysUpTime` 等 | `read_only` |
| SNMP GetNext | 查询下一个 OID | MIB 浏览器选择对象 | `read_only` |
| SNMP GetBulk | 批量读取 varbind | 表和子树查询 | `read_only` |
| SNMP Walk / BulkWalk / TableWalk | 遍历子树或表 | `ifTable`、`ifHCInOctets`、`lldpRemTable`、`entPhysicalDescr` | `read_only` |
| SNMP Set | 写入可写 OID | 必须先显式启用 SNMP Set | `config_write` |
| SNMP Trap / Notification | 接收 Trap | Trap OID、告警规则 | `read_only` |
...
Electron Desktop：页面路由已实现，Feature/导航暂不开放（`BLOCKED_ON_TASK_WINDOW`）
```

该页面只读展示，不执行任何命令。Qt 与 Electron 均支持搜索、筛选、查看详情、复制命令模板和刷新；Electron 搜索按 250ms debounce 对齐 Qt `textChanged`，并以请求代次丢弃过期响应。Markdown 导出复用既有 Export Process、`TaskDTO/TaskCancelResponse` 和公共 `WebArtifactStore`，公开 Task/下载文件名固定为安全可读 `.md` 名称，不公开 UUID 物理名或路径。模块只用严格命名的 localStorage key 保存当前任务 ID，通过既有 `/exports/{task_id}` 串行轮询；完成、失败、取消和页面卸载均停止轮询，损坏或 404 的恢复 ID 会被清理。

当前仍为 `BLOCKED_ON_TASK_WINDOW`：共享 `TaskWindowContext`/validator、Job Center 模块筛选与取消 owner capability 尚未接受 `command-reference`。模块不再转换或扩张 Electron bridge 类型，而是通过正式 PlatformAdapter 仅传当前公共类型允许的 `{taskId,status}`；浏览器开发态才回退 `/tasks` 并携带模块查询参数。待共享 allowlist 合入后，由集成单写者在正式类型和 validator 中补充 module。页面文本沿用仓库现有 `navigator.language` 翻译器模式，并优先读取 `document.documentElement.lang`；待系统设置 v2 的全局动态 locale runtime 合入后，只替换该语言来源，不在模块内建立第二套全局 locale store。
