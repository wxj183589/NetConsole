...
Electron Desktop：左侧导航 -> 命令说明
```

该页面只读展示，不执行任何命令。Qt 与 Electron 均支持搜索、筛选、查看详情、复制命令模板和刷新；Electron 的 Markdown 导出已接入既有 Export Process、持久 Task、取消和重启恢复，但生产 Artifact 下载仍为 `BLOCKED_ON_TASK_WINDOW`，等待统一任务窗口在公共 `WebArtifactStore` 登记 `command_reference_export` 来源后接通，不得在模块内复制 Artifact store 或任务最终化协议。
