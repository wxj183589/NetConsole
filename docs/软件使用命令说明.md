...
该页面只读展示，不执行任何命令。Qt 与 Electron 均支持搜索、筛选、查看详情、复制命令模板和刷新；Electron 搜索按 250ms debounce 对齐 Qt `textChanged`，并以请求代次丢弃过期响应。Markdown 导出复用既有 Export Process、`TaskDTO/TaskCancelResponse` 和公共 `WebArtifactStore`，公开 Task/下载文件名固定为安全可读 `.md` 名称，不公开 UUID 物理名或路径。

当前仍为 `BLOCKED_ON_TASK_WINDOW`：共享 `TaskWindowContext`/validator、Job Center 模块筛选与取消 owner capability 尚未接受 `command-reference`；模块已按 `{taskId,module,status}` 调用 Electron bridge，浏览器开发态才回退 `/tasks`。页面文本沿用仓库现有 `navigator.language` 翻译器模式，并优先读取 `document.documentElement.lang`；待系统设置 v2 的全局动态 locale runtime 合入后，只替换该语言来源，不在模块内建立第二套全局 locale store。
