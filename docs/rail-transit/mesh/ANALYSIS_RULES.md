...
- 当前派生库 schema 为 `meshlog_compact_v3_tagged_samples`，当前 parser 语义版本为 `meshlog_parser_v1`，中央 catalog schema 为 `mesh_catalog_v3_source_lifecycle`。历史版本曾把 schema 字符串写入 `parser_version`；该值只作为兼容别名读取，不能据此提示新来源升级。新导入必须直接写入当前 schema、parser 和 Identity revision。同一毫秒内日志携带的 `(2)`、`(4)` 等 `timestamp_tag` 属于采样身份的一部分，解析、唯一键、链路分组、质量分析和报告不得把它们合并。
...
- 页面打开来源只读取上述版本和 revision，不自动提交任务。`identity_projection_refresh` 使用 `force_reparse=false`，`parser_rebuild` 使用 `force_reparse=true`；二者都通过单一 typed maintenance API 和 Job Center 显式提交。身份 remap 的记录数守恒以写入 detail SQLite 后的去重链路数为准，不能用 parser 原始列表长度与 `INSERT OR IGNORE` 后的记录数比较。
...
- 来源列表、批量操作和当前详情提供显式来源删除。单来源使用 `mesh_analysis_source_delete`，多来源必须合并为一个 `mesh_analysis_sources_delete` Job 并在 Worker 内顺序处理；不能由 Renderer `Promise.all` 并发创建 N 个任务。两者均支持仅删除 parsed/detail、mapping/cache 与可选关联报告并保留归档 raw，后续走显式重建；或删除当前数据根内归档 raw、全部派生结果、指纹和关联报告，使同一外部文件可以重新导入。删除前必须二次确认，活动导入/解析/重建/报告/同源删除任务会由 `mesh_catalog:<site>`、`mesh_profile:<site>:<profile>` 和 `mesh_source:<session>` 资源键阻断。
...
- Profile 根、raw、parsed、报告或整个受管目录已被人工删除时，删除与重新导入都必须幂等。导入 preflight 把失配持久化为 `BROKEN_SOURCE`，重导同正文恢复同一 session；目录创建统一使用受管 Storage/PathResolver 且 `parents=True, exist_ok=True`。当前数据模型仍是“一条 source 即一条 analysis session”，因此本轮只开放安全恢复/覆盖；在独立 analysis-run 模型落地前，不伪造“同一内容创建新分析记录”。
...
- `apps/desktop_renderer/src/components/rail-timeline/RailRssiComparison.vue` 是轨道交通 RSSI 双图公共布局，`railTimeline.ts` 是 viewport/cursor/selectedTime 控制器；离线 MESH 与 Online MR 都复用它们。共享层只理解标准时间域、图表状态和 slot，不依赖 MESH/Online MR API、parser 或数据库 DTO。`MeshRssiChart` 与 `MeshTracksideSignalChart` 仍保留既有 MESH 数据语义，抽取公共框架不得改变原页面查询、降采样、切换映射或缓存边界。
- 页面按源文件解析到实际的 compact v3 tagged samples 明细库，直接读取版本化标量列；不在正式查询路径回退旧 JSON 指标列。
- 主链 RSSI 图的数据精度与业务 Overlay 分离。默认 `resolution_mode=full` 时，Repository 按 Radio 和可选时间范围从 `samples LEFT JOIN mesh_links` 读取全部有效 ACTIVE MR 侧 RSSI，并以紧凑 `[timestamp, rssi, gap_before]` 数组返回；该主线不进入 `critical/trend/ordinary` 选点、`max_points` 或 ECharts sampling，返回数必须等于范围内有效样本数。切换、三角链路、异常、站点/区间、Peer RSSI 和 Tooltip 上下文继续使用富 `MeshChartPoint` 与独立 Overlay Budget。`high/overview` 模式及空口负载仍使用三级采样：一级为首尾、有效切换/快速回切、真实缺口、持续 0 边界和短时 0 后恢复点，以及每个连续 `NO_ACTIVE`/`MULTI_ACTIVE` 状态段的起止边界；二级为链路/角色/区段边界及按剩余预算自适应时间桶保留的 RSSI/Busy 峰谷；三级为自然秒真实代表点。统一 `MeshChartResponseBudget` 继续限制富 point/event/location/series/frame/annotation 规模，响应目标 4 MiB、硬上限 16 MiB；不得以 Overlay 的 `total/returned` 冒充 Full 主线统计。
...
- `active-path` 首次打开默认按完整日志域请求 Full 主链，`trackside-signal` 仍按完整日志域请求受预算约束的概览。RSSI 父页明确分离完整日志 `fullDomain`、本地实时 `previewViewport`、最近发布成功的 `committedViewport` 和待查询 `queryRange`：Full 主线加载后保存在 `shallowRef/markRaw`，用户缩放只由客户端 DataZoom 裁剪，不重新请求或替换主线；轨旁图在 pointer/touch 结束 200 ms 或无可靠 pointerup 的 450 ms idle 后请求最终窗口。程序化镜像同步、布局切换、Resize、共享 pointer 和 KeepAlive 恢复不得触发主线查询。
- `high/overview` 精度兼容原同窗口批次语义：主链与轨旁请求使用相同 session、Radio、`time_from/time_to` 和 generation，两个结果与新轨旁 cache 全部就绪后成组发布，失败或 Abort 保留旧图。Full 模式中只有轨旁窗口响应替换轨旁 cache，主链继续使用完整数据；重置视图恢复客户端完整主线 viewport，并按需重载轨旁完整域概览。
...
MESH catalog、Profile `mesh.sqlite` 与来源 detail SQLite 分别保存会话摘要、来源索引和解析事实。物理目录被外部删除时，查询不得重建目录或把单个悬空来源扩大成页面 500：整个根缺失返回空快照，单个 detail DB 缺失只过滤该来源；正式删除仍需清理 catalog/source 关系并区分已删、缺失和幂等重复。

来源保存 `identity_index_revision`、映射时间和状态。普通 GET 只读比较 revision，不重建 Identity；健康 parsed DB 通过 identity-only remap 批量替换 Peer 映射与身份投影，不重解析 raw、不改变链路、ACTIVE/STANDBY、切换或 RSSI 事实。缺失、损坏或旧结构 detail DB 才进入 raw 重建。索引和基础资料不完整时保持 `unresolved/ambiguous`，不得恢复前缀、OUI、名称或位置模糊匹配。

导入顺序固定为：校验预览与映射，刷新 stale 的当前局点派生 Identity，复制一致的数据库与 catalog 到 staging，在 staging 中解压/解析/精确映射，最后原子发布 Profile、catalog 和成功 manifest。普通查询不承担写入补偿。

重型图表先加载主链首帧，再按可见性和 idle 门加载轨旁图；两者具有独立 loading/error/retry。相同请求 key 复用在途 Promise，新 session/Radio/revision 通过 Abort 与 generation 阻止迟到响应。KeepAlive 暂停只停止页面轮询和瞬时 pointer，关闭标签或切换局点才释放 ECharts、series cache、外部 Map 和请求。
