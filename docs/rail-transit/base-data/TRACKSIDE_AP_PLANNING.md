...
实际上线数量复用现有 FIT-AP 在线状态，但 AP 规划维护中带有效 `station_id` 的逐站规划是唯一业务统计边界。基础 AP 资料中的完整 MAC 与稳定 `station_id` 是首选关联；没有任何基础 AP 资料命中时，允许使用“当前车站交换机 LLDP 完整邻居 MAC = FIT-AP 完整 MAC”的唯一精确证据进行只读运行态站点投影。只有投影结果的 `station_id` 同时存在于当前逐站规划中，AP 才能计入该站上线数量。站名、VLAN、AP 名称和邻居 IP 不得扩大统计范围；参考资料记录数本身不作为统计字段或上线率分母。

FIT-AP 运行态只把 AC 的 `R/M`、`R/B` 计为在线，其余明确状态计为离线；状态字段全部缺失时保留未知且不计入在线。轨旁业务明细只对最新有效 AP Rx 使用固定维护线：严格低于 `-13.90 dBm` 标记“光衰大”，等于该值仍为正常，空值、无效值或过期值为“光诊断未采集”或未知。型号去除首尾空格并忽略大小写后等于 `WA6522` 时，AP 光诊断固定为“不适用”，不读取交换机侧数值反推 AP 光衰，也不进入光衰异常数量、筛选或异常导出。

```text
未上线 = 规划 AP 总数量 - 规划范围内实际上线数量
站点上线率 = 实际上线数量 / 规划 AP 总数量
总上线率 = 实际上线合计 / 规划 AP 总数量合计
```

规划 AP 总数量为 `0` 时上线率为空，由页面显示为 `—`。逐站实际上线按规范化稳定 AP MAC 去重并封顶到该站规划数量；原始观测超过规划值时保留“超规划”诊断，但页面与导出的上线数不超过规划数，上线数加未上线数始终等于规划数，上线率不超过 `100%`。缺少规划的站点不生成业务统计行。有规划但没有匹配 FIT-AP 时实际上线为 `0`、未上线等于规划数量。未关联当前规划有效 `station_id` 的 AC 在线 AP 仅作为数据质量提示及分页诊断，不生成“等待 LLDP 同步”“基础资料待补充”或“当前 LLDP 冲突”业务统计行，不进入分站、合计、上线率和业务统计导出。概览和导出合计备注可同时列出 AC 全量资源、已关联上线与未关联数量，但必须明确未关联 AP 未计入业务统计。AC 管理 / FIT-AP 资源仍是全量 AC 资源视图，不受轨旁规划过滤。
...
完整主数据边界见 [轨旁 AP 主数据与关联模型](../TRACKSIDE_AP_DOMAIN_MODEL.md)。
