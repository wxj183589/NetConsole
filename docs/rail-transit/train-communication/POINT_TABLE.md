...
“生成当前列车六节点点表”任务只返回编辑区预览，任务参数固定为 `save_result=false`，不会改写正式点表。任务查询仅对 `car_network_generate_point_table` 受控返回规范化后的点表字段、节点数和等待保存说明；其他任务不会因结果中包含同名列表而公开节点数据。生成结果缺失、不是列表或包含无效节点时，页面必须显示生成结果不可用，保留当前编辑内容，不能把 `COMPLETED` 解释为预览成功。
