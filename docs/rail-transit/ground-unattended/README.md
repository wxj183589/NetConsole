...
- 当前运行与最近终态分开查询；Profile 禁用、等待窗口、运行中和终态不能互相覆盖，Backend 恢复必须收敛无法恢复的运行记录。
- Ping/Syslog 查询先按 run、列车、目标、时间和文件索引预筛，再流式读取并返回扫描预算与 `truncated`；丢包、切换和位置边界在降采样中优先保留。
- 原始 NDJSON 是事实源。ACTIVE/OPEN 文件、正在最终化或归档的 run、READY ZIP 成员都禁止记录级改写；删除必须预览、短期 token、revision/hash 复核、同目录原子替换、Repository 单事务和审计。
- READY ZIP 读取与下载必须验证受管路径、成员规范、数量、解压大小、压缩比、CRC、登记大小和 SHA-256；Renderer 只提交 opaque archive ID。
- Syslog 设备时间与本机接收时间分开保存；未知或冲突身份不绑定 MR，Peer/BSSID 多候选保持 `AMBIGUOUS`，不能按名称、序号、站点或 MAC 相似度猜测。
- 页面隐藏或卸载只停止查询和图表资源，不停止 Supervisor、AC 轮询、fping、Syslog 或深度采集；所有请求使用取消、generation 和过期响应丢弃。

真实 AC/MR、长时间 fping、多列车 UDP、托盘隐藏运行、低磁盘、正式安装包和多缩放 Electron 视觉仍是独立人工门，不能由隔离数据或单元测试替代。
