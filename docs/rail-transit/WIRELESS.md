...
轨旁 AP 业务通过 `TracksideSwitchAdapter` 隔离厂商差异，H3C 保留原有采集路径，ZTE ZXR10 使用独立只读 Adapter。ZTE 普通设备详情固定执行 Profile v3 的七条只读命令，默认不按端口追加光模块 detail；轨旁更新使用每设备一个 SSH 会话，固定执行 `show version`、`show interface brief` 和 `show opticalinfo brief`，LLDP 关系读取数据库已有快照。接口或光模块摘要无效时保留历史事实但不再作为本轮实时状态展示，任务以结构化告警收口。两条路径都不进入配置模式，不提供配置采集、配置下发、文件管理、CLI Ping 或 ZTE AC 能力。
...
轨旁 AP 业务只对 AP 接收光功率使用固定业务门限：最新有效 AP Rx 严格低于 `-13.90 dBm` 时为“功率异常”，恰好 `-13.90 dBm` 仍为正常；空值、无法解析或过期数据为未知。交换机侧继续使用交换机光模块自身 RX/TX 门限和设备告警，不套用该固定值。页面、筛选、统计、单 AP/站点/全部更新后的重建和业务导出共用同一 AP 业务判定，同时分别展示 AP 设备模块状态；端口 `DOWN` 仍在业务综合状态显示“链路断开”。该规则不修改 AC/FIT-AP 资源、设备详情、通用光模块状态或 ZTE 厂商原生门限。
