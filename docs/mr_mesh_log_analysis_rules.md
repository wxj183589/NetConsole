...
- RSSI 页使用一个按实际剩余视口高度计算的图表工作区，并提供“对比 / 主链 / 轨旁”三种布局。对比模式在同一工作区内纵向分配两图，默认 50/50，可用 Pointer Events 拖动分隔条并在每帧合并 resize；常规最小 pane 高度为 280px，较矮窗口降为 240px，空间仍不足时只允许工作区内部有限滚动。专注模式以 `v-show` 保留另一图的数据、ECharts 实例、series cache 和 viewport，隐藏图停止 pointer、Tooltip 与 resize，恢复显示后只执行 `resize()` 和静默共享 viewport 应用。布局模式和 0.25～0.75 的对比分隔比例只通过白名单 UI preference Bridge 或浏览器 localStorage 保存，不进入业务数据库；布局切换不得重新调用图表 API、重建 series 或修改共享时间域、pointer、切换标记和最小 1 秒视口。
...
- 轨旁 payload 使用 `shallowRef + markRaw`，到达时只遍历一次并建立非深度响应式缓存。ECharts 点固定为 `[timestampMillis, rssi, metaId, roleCode]`，不得携带完整 point、series、`seriesMeta` 或 `points`；Tooltip 必需的紧凑标量元数据、dataIndex 和 frame 索引保存在 ECharts 外部 Map。viewport、位置带、主题和 resize 复用同一紧凑 data，dataZoom 不执行完整 `setOption`，不再复制或排序全部 AP 序列。轨旁 Tooltip 只按当前 frame 索引读取并显示一次时间、AP/Radio、角色、轨旁/MR RSSI、站点/区间及有效 ACTIVE 持续时间；同半区内固定在对侧上角，多链路时内部滚动且滚轮不冒泡到图表。小数据下 ACTIVE 使用实心圆、STANDBY 使用空心圆；5,000 点以上仍关闭业务曲线 animation、symbol、emphasis 和每点 label，但切换信息始终只使用一个 scatter 数组和首条业务 series 上的一份 markLine，展示开关通过轻量 overlay `setOption` 更新，不重建缓存或业务 series。轨旁图接近可视区域时才初始化 ECharts，但数据请求仍提前完成，初始化后立即应用当前共享 viewport。系列颜色使用 Peer Radio MAC、AP MAC、Peer MAC、序列身份与 Radio 的稳定优先级计算，不含点角色。
