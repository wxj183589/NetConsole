...
- 主链 RSSI/Busy 图按时间窗口和 Radio 查询，显示层优先按自然秒保留一个真实代表采样，安全上限为 20,000 点；20 MiB 级原始日志先在服务端按秒选择真实点，不把同一秒的高频原始行全部传给 Renderer。只有自然秒数或关键毫秒级点超过上限时才按时间抽样。服务端保留首尾、有效切换/异常/无 ACTIVE 断点、短时 0 后首个恢复点和极值；关键点提高本次有效点数时，仍在安全上限内额外保留按时间分布的普通代表点，不能让关键点占满预算后只剩长区段首尾。DTO 返回 `total_points/returned_points/downsampled`、请求/有效点数和必要的 `downsample_warning`。
...
- 轨旁接口的 `max_points` 表示目标采样时刻数，兼容字段 `requested_max_points/effective_max_points` 与新的 `requested_max_frames/effective_max_frames` 同值，不是链路点硬上限。后端先按 `(source_file_id, sample_time, timestamp_tag, local_radio)` 建帧，并优先每个自然秒保留一个真实 frame，再选择首末、序列/区段边界、角色/主链切换、异常、缺口前后、短时 0 后首个恢复帧和区段 RSSI 极值等关键帧；一旦选中某帧，必须返回该帧全部有效主备链路。关键帧达到或超过请求目标时继续增加按时间分布的普通代表帧，避免长 run 只保留首尾或隐藏 0 后丢失恢复点。DTO 分别报告 frame、链路点、序列和链路存在区段数量，关键帧超过请求目标时提高有效 frame 预算，不截断整条 AP 序列或同帧链路。
