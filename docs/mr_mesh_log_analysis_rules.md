...
- 主链 RSSI/Busy 图按时间窗口和 Radio 查询，默认按请求目标点数降采样，硬上限为 2,000 点。服务端保留首尾、有效切换/异常/无 ACTIVE 断点、短时 0 后首个恢复点和极值；关键点提高本次有效点数时，仍在硬上限内额外保留按时间分布的普通代表点，不能让关键点占满预算后只剩长区段首尾。DTO 返回 `total_points/returned_points/downsampled`、请求/有效点数和必要的 `downsample_warning`。
