...
V1 已实现 iPerf server/client、真实 fping、并发 TCP fallback `ping_probe`、MR 持久 SSH Shell 原始日志采集、目标管理、统一任务生命周期、增量事件、结果描述、优雅退出和停止后 ZIP 包。最终任务状态在打包提交后发布。Agent 暴露 HTTP REST API，并内嵌同一套 API 驱动的轻量 Web 页面。完整构建、运行、配置、API 和限制见 [Agent README](../agent/README.md)。
