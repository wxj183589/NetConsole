...
- 设备管理第一批 Electron/Qt 对等整改：删除旧只读校验链，编辑改为真实保存并同步详情；秘密字段增加保持、替换、显式清除三态且不回显；CSV 导入增加已有主地址重复行预览及拒绝/跳过/仍新增策略；设备页面移除私有 `sessionStorage` 任务记录。统一任务窗口仍依赖 `codex/electron-task-center-window@fe2c54e8`，未保存表单的真实连接测试仍缺一次性秘密注入契约，因此设备模块保持 `PARTIAL / BLOCKED_ON_TASK_WINDOW`。
