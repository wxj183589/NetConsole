...
- 修正轨旁 AP 业务导出上线概览的合计口径：未关联有效站点的在线 AP 以“基础资料待补充”独立行列示并计入实际上线合计，合计备注同步显示 AC AP 资源、已关联上线和待补充数量，避免导出只显示已关联数量而被误认为旧数据。
...
- 统一 FIT-AP 业务与轨旁 AP 业务的 AP Rx 判定：最新有效值严格低于 `-13.90 dBm` 时标记“功率异常”，等于边界值仍为正常，空值、无效值或过期值为未知；AC/FIT-AP 资源、设备管理和交换机侧继续使用各自模块门限，并与业务状态分列展示。
