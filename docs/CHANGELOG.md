...
- 基础资料新增只读完整编辑快照：点击“编辑”后在同一 revision 边界读取全部可编辑实体，彻底隔离查看列表的分页、搜索与筛选；快照不加载运行态、不重建 AP Identity、不写数据库，失败时保持查看态。同类查看刷新复用并等待真实在途请求，内部标签切换继续保留唯一草稿。
- 轨旁 AP 规划资格与主线区间拓扑资格拆分：启用的普通站、车辆段和停车场都生成逐站规划，车辆段/停车场保持节点类型且不生成主线区间；10 个普通站加 1 个车辆段稳定得到 11 行规划，新增特殊节点默认 AP 数为 0、VLAN 为空。
- 基础资料页改为显式 `VIEW / EDITING_* / READ_ONLY` 交互：可写会话默认以纯文本和状态标签查看，点击“编辑”后才创建统一草稿；保存或取消后返回查看态，保存失败保留草稿。轨旁 AP 规划同步采用受控双态渲染，查看态不再显示输入、选择和增删入口。
