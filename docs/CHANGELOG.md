...
- 轨旁 AP 规划从“每站保存一套管理 VLAN”统一重构为 AP 管理 VLAN 分组模型；全线统一、每站独立和连续 1～4 站分组共用同一套分组、有效网络、地址分配和校验流程，全线统一模式不受 4 站上限限制。
- 规划页新增 VLAN 组与站点继承两级视图，支持自动分组、拆分、相邻合并、成员边界调整、区间默认组和 AP 级覆盖；模式或网络参数变化先显示站点、AP、VLAN、IP、手工地址及冲突影响，默认保留锁定或手工地址。
- `devices.db` 新增线路规划、VLAN 组、稳定站点成员、区间/AP 归属和地址分配表；旧站点级规划幂等迁移为每站一组，并继续保留兼容投影，规划与基础资料保存使用同一事务和 revision 冲突控制。
- 点表和 PVID 核验统一读取已识别 AP 的有效 VLAN 组；不同站点复用 VLAN、网关或子网只提示，真实 IP 冲突仍阻断，不再通过 VLAN 反推站点或把跨站相同 PVID 判为冲突。
