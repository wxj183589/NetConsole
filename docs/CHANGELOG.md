...
- FIT-AP 行右键菜单新增桌面版受控外部终端，复用系统终端设置和设备连接目标；Browser/Server、离线/无 IP/无唯一凭据场景失败关闭，API 不接受可执行文件、参数或密码。
...
- 在线列车车内通信检测页改为结构化业务表查询，“无线统计”不再走旧指标接口。
- 修复当前会话回落与业务表分页链路，未生成 parsed 数据库时仍可继续查看原始日志和已结构化业务区。
- 在线列车下拉框改用正式列车在线状态 `/train-communication/online`，由 `VehicleMrOnlineQueryService` 汇总 AC Mesh-Link 与列车 MR 映射；`active_sessions` 仅保留为辅助信息，不再决定列车是否可选。
- 增加统一列车身份规范化，`列车01`、`01车`、`01`、`1`、`train-01`、`train:01` 和 `LC01` 等常见格式会归并到同一 `canonical_train_id`，用于在线状态、点表、拓扑和诊断结果匹配。
- 点表管理完成“生成预览 -> 保存点表 -> 原子替换 -> 重新读取 revision -> 通知父页面 -> 刷新拓扑 -> 允许立即检测”的闭环；当前列车无点表时可直接生成六节点骨架，MR 节点优先绑定正式 CT/TC MR。
- “立即检测”提交前由后端再次复核当前列车仍为双端或单端在线且数据新鲜；列车离线或数据过期时返回 409，避免使用旧选择启动诊断任务。
