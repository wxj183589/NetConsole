...
- 修复设备管理默认排序：完整筛选结果在分页前按 `COCC > BOCC > 车站 > 车载-MR > 车载-SW > 其他 > 未分组` 排列，组内设备名称使用数字自然排序；兼容历史分组别名并保留原显示名称，显式排序仍可临时覆盖默认顺序。
- 系统设置的历史数据库备份支持按稳定 `backup_id` 多选、全选当前完整结果和一次性批量删除；删除前展示数量及预计释放空间，后台只提交一个批量任务并返回逐项 `deleted/failed/skipped` 结果，部分失败保留失败项选择。
- 新建设备和空白导入字段统一默认建设阶段为一期（`phase_1`），保留已有设备及复制设备的实际分类值，历史未指定数据不自动迁移。
- 将车载 MR 统一投影为 Device → VehicleMr → Train 关系；兼容 `LCxx-MR-CT/CW`、站点列车号和历史分组别名，基础资料及列车在线/通信消费者共享同一来源，并对无法解析和重复位置生成质量问题。
...
- 修复 MESH 原始日志刚导入完成后即被判定为 AP Identity 映射过期的问题。导入解析完成前自动收口到当前 AP Identity revision，并校验投影未改变解析事实，不再要求用户对新日志额外手工刷新。
- 保留历史日志在 AP 身份资料后续发生变化后的过期提示和手工刷新机制；AP Identity revision 在投影期间持续变化时有限重试，无法稳定则导入失败，不伪造 ready。

## v1.5.3 - 2026-09-02

### 优化

- 优化轨旁 AP 规划的排序、编辑、粘贴和校验定位，并统一正线与非正线节点的稳定顺序。
- 优化 Task Center 结果缓存与清理扫描，降低长期运行开销并保留受控的结果引用边界。
- 优化站点维护与数据维护入口，补齐空壳局点和外部引用的安全处理边界。

### 修复

- 修复轨旁 AP 光衰同步、状态展示和 Excel 导出不一致的问题，保留完整处理事件历史。
- 修复 Task Center 多局点路由、孤儿任务恢复以及清理后晚到写入可能造成的状态错乱或任务复活。
- 修复 MESH 路由缓存、原生标题导航和原始日志批量 MR 映射状态不一致的问题；明确选择 MR40-CW 后，页面校验和提交 payload 均使用对应内部 Profile，归属其他 MR 的重复内容继续 fail-safe。
- 修复 AP Identity 初始化无变化写入导致 revision 异常增长的问题。
- 修复 Ground Ping 重启前后历史分段查询的时钟边界回归，确保跨分段查询的 union、顺序和去重契约持续有效。
...
- 支持多 AC 独立管理：每个 AC 独立维护自己的 FIT-AP，站点级轨旁 AP 业务可聚合多个 AC 的结果。
- 完善 FIT-AP 物理身份识别，有效序列号作为强物理身份，避免多 AC 或重复采集产生重复 AP。

### 改进

- AP 上线情况概览移除“再上线数”和“再上线率”，简化站点概览并增加按轨旁 AP 业务口径统计的光衰问题数和告警状态。
- 新增 AP 上线检测统一依据 AC 未认证 AP 列表，避免普通上线或重连被误判为新增 AP。
- 优化轨旁 AP Excel 导出：光衰问题数大于零的概览单元格使用浅红提示，主业务表移除识别诊断列，AP 光衰处理记录改为导出完整事件历史。

### 修复

- 修复工作区从设备管理等页面直接进入 MR 原始 MESH 时缓存组件身份错配，避免标签和标题已切换但内容仍停留在旧页面；保留既有 KeepAlive，并在路由组件身份变化时重建对应缓存包装器。
- 修复开发模式下数据根和运行模式拼接后的原生窗口标题超过 Electron Bridge 长度契约、进而阻断 MESH 路由导航的问题；标题仍保留页面名称并限制在桥接契约范围内。
- 修复 MESH 原始日志导入中批量选择车载 MR 后仍保留旧自动映射的问题；选定 MR 会按精确关联同步/复用内部 Profile，直接 `.log.gz` 文件和三文件批次均使用同一有效映射提交，已归属其他 MR 的重复内容继续保持冲突保护。
- 修复 FIT-AP 采集失败时错误更新或清空 Current 的问题；只有确认完整成功且确实为空时才清空 Current，并明确区分成功有数据、成功为空、失败和未采集。
- 修复多 AC 环境下 AP 序列号、APID 与物理身份作用域不一致，以及全局有效序列号重复写入的问题。
- 修复 AP 光衰处理历史被单行当前摘要覆盖、复发异常无法分别保留的问题，并补齐记录中的序列号和正式关联信息。
- 修复站点光衰问题数与轨旁 AP 业务异常总数不一致的问题；站点统计和业务页使用同一光衰判定口径。

### 数据与兼容性

- 新增 `2026.09.01.ap_optical_treatment_event_history_v1` schema migration，并提供旧光衰处理数据向事件历史模型的受控迁移能力。
- 保留 `ap_optical_treatment` 当前摘要查询模型；事件历史独立保存每个异常生命周期，升级不改变当前 AP 光衰状态的读取方式。

### 工程维护

- 完成旧外部 HistoryStore 运行时退役，当前相关数据统一使用 Current、Recent10 和既有有界历史架构；Task Center 的 TaskHistoryStore 保持不变。
- 发布与质量验证统一到本地发布门禁，GitHub Actions 不再作为发布依据。
...
- 修复生产切换后的重启恢复锁，避免恢复流程重复执行或遗留错误状态。
- 完善生产任务结果最终化回填与无效任务保护，确保任务终态和结果数据一致。
- 修复生产历史退休批次扫描，补齐数据库治理与任务存储集成的收尾路径。
- 合并数据库治理、生产维护门禁、设备历史存储 V2 与旧历史 COPY-only 迁移能力。
- 合并任务存储治理、终态结果消费者矩阵与进度事件写放大修复。
- 合并地面无人值守现场回归、Electron 打包门禁、构建身份和轨旁 AP WPS 交付修复。
...
- 基础资料总览、站点与区间、轨旁 AP、轨旁 AP 规划和列车与车载 MR 统一恢复为默认纯查看；用户仅在点击当前子页“解锁当前子页”后才建立该 scope 的 revision 草稿并显示维护控件，保存或放弃后回到查看态。总览现在也可在解锁后维护线路和方向参数，局点 ID、来源、更新时间和说明保持只读；revision 冲突只能刷新后重新解锁。本次未修改数据库 schema。
- 2026-08-07 本轮“轨道交通 / 基础资料”人工验收通过：根入口进入总览查看态，五个可维护子页均按当前子页独立解锁、保存和退出编辑，诊断、预览、审计与关联运行状态页保持只读。

### Electron Runtime 与导出恢复
...
- 2026-08-07 Electron 人工验收通过：AP 上线情况概览保留顶部全局摘要，站点表增加固定底部合计行；规划 AP、实际在线、离线和上线率直接使用 online-status API 的全局统计，合计状态和告警沿用统一口径，并与导出 XLSX 的合计一致。弹窗支持拖动、缩放，表格在尺寸变化时保持内部滚动和列对齐。
...
- 建立统一数据库升级框架，通过注册表、备份、日志、历史和 SQLite 一致性检查管理升级；系统设置可查看计划与历史、启动受控升级任务，失败时保留备份和可追溯记录。
- MESH 解析库恢复、源文件重建和派生数据维护接入统一升级与侧车路径，可识别已归档原始日志与解析库的不一致并安全修复，不删除原始日志。
- Online MR 历史会话新增 capability 驱动的解析库自动升级：终态 Session 打开后自动复用 `online_mr_parse` Job，从本地 raw 构建临时库，经完整性、metadata、raw fingerprint、业务表与投影校验后原子切换；失败保留旧兼容结果，相同缺失/损坏 raw 不重复创建任务，并保留单一回滚副本。该能力当前是 Online MR 领域内闭环，尚未声明为全局数据库升级 adapter。
- 任务中心和 MESH 时间展示统一按业务时区格式化，修复 Windows 下 UTC 时间与本地业务时间混用导致的显示偏差。

### Online MR 分析工作台

- 动态图统一为左侧固定分析信息栏和右侧纯图表区，所有动态图 Tab 在同一位置展示时间校正、当前时刻、主链路、悬停指标、切换事件和解析状态；图上 Tooltip 收敛为轻量值提示，单击锁定分析时刻，解除按钮或 `Esc` 恢复 Hover 跟随，窄窗口使用覆盖式抽屉。
- 普通与沉浸模式改用剩余 viewport、响应式网格和图表区内部滚动分配主链路、轨旁 AP 与关联指标高度；真实容器 `ResizeObserver` 驱动 ECharts resize，避免父容器变化时 X 轴或 DataZoom 被裁剪。自动升级完成只刷新当前 Session 的解析数据，并保留活动 Tab、Zoom、锁定时刻和沉浸状态。
- Online MR 动态图关闭脏矩形重绘并将 HTML Tooltip 约束在图表容器内，指标切换、Resize 和停用时主动清理悬浮层；新增项目级动态图稳定性 Skill 与架构门禁，持续检查实例释放、空值断线、ResizeObserver 和 series/DataZoom 替换策略。
- Ping 质量的 RTT、丢包率、固定分析栏、Tooltip 与 Y 轴改为复用同一 metricId 单位契约：RTT 固定为 `ms` 且不再受百分比上限裁剪，丢包率继续为 `0~100%`；Tooltip 按“时间/目标/指标”分字段显示，不再拼接 IP、数值和单位。
- 主链路与轨旁采样中的设备命令 RSSI 原始值按无单位信号值展示，切换事件中明确为物理量的字段继续保留 dBm，避免把 `47~50` 等原始值误标成正 dBm。

### 轨旁 AP 当前异常光衰

- “当前异常光衰”改为按每个 AP/交换机接口的最近一次有效光衰事实判断，数据过期不再等同于正常或无数据；本轮采集失败、超时或无光衰字段时沿用最近有效缓存，更新的正常值仍会覆盖旧异常，明确 `no_module` 和不适用型号继续过滤。
- 本地 XLSX 与 WPS 共用的 Workbook Snapshot 保留最近有效观测的真实采集时间，不伪造当前时间，也不修改现有列契约或 WPS AirScript。

### 设备导入与 AP 拓扑

- 设备导入支持保留尚未适配的厂商和型号；这类设备可进入台账与导出，但连接测试、批量采集和详细采集会以“厂商未适配”明确跳过，不会误发 H3C 命令。
- 新增以设备事实优先的 AP 拓扑解析器，统一处理 AP/Radio/BSSID 及站点关联；AC 查询、MESH 位置与报告复用同一证据顺序，歧义和缺失 RSSI 保持可诊断，不做名称模糊回退。
- 修复 AC Mesh-Link 拓扑字段为空时的兼容回退，保留有效的 MR、Peer、Radio 和链路证据，避免空字段导致拓扑误判。
- 修复 MESH 来源重建复用旧 AP 拓扑投影的问题：索引会识别拓扑投影版本过期，在受控重建后重新投影站点和区间；普通查询保持只读，不写入业务数据库。
- MESH 重映射摘要新增站点/区间的已解析数、未解析数、来源分布和拓扑告警计数；LLDP 只有稳定站点 ID 而缺少显示站名时保留可诊断告警，不猜测站点名称。
- 2026-08-07 现场人工验收通过：本次 MESH 来源重建已恢复站点投影且保留原始链路事实；未提供现场样本的其他局点不在本次结论范围内。

### 地面无人值守证据链

- Current AP 与 WMESH 主链路切换统一复用 AP Identity 的物理 AP、Radio、BSSID 和已登记 alias 精确解析；Ping 曲线从持久化 `MESH_ACTIVELINK_SWITCH` 投影独立事件标记，悬浮信息包含切换双方 AP/站点、同列车同 MR 的真实 RSSI 证据及 Syslog 来源。无匹配采样明确显示，不插值生成 RSSI。
- 深采页面不再把“符合资格”显示为“正在采集”：只有 Online MR 会话可读取且已有 Collector 原始数据写入时才显示 `RUNNING`。新增受管会话 raw/log 的 cursor 增量查看接口；窗口暂停仅暂停前端刷新，不会停止后台采集或创建新的 SSH 采集路径。
...
- 新增 Windows 一键 Full/Customer 双版本打包入口，补齐依赖检查、阶段日志、运行时恢复、并发日志归档、`pnpm` 环境回退和 Windows 双击换行兼容。
- 修复 Full/Customer 功能模板生产能力缺失、客户版 smoke 误用 Full 门禁及临时 `pnpm` 代理被清理的问题，双版本制品现在按各自固定功能基线验收。

## v1.4.7 - 2026-08-05

### 版本与功能交付

- 删除系统设置中的完整运行时功能矩阵，改为只读“当前版本状态”和退出会话预览、清理历史覆盖、重新加载 Gate 等显式维护动作；Full/Customer 模板统一在“版本与功能交付”维护，会话预览不再持久化运行时覆盖。
- 客户版配置合并为“不交付 / 交付并显示 / 交付但隐藏”三态，业务模块、页面操作和只读技术能力分层展示；依赖问题由 Backend 结构化返回并支持只修改草稿的自动补齐。
- Feature Registry 拆分界面父级、运行依赖和交付依赖，并补充共用技术 Capability；修复客户模板的列车在线、轨交任务控制和 Online MR 操作交付闭包。
- Full/Customer 版本资源注入前增加 Profile 完整性、依赖闭包和内部/非正式功能泄漏校验，异常时构建直接失败并输出具体链路。
...
- 2026-08-06 Electron 客户端人工验收通过：系统设置可进入局点维护，局点信息展示/编辑和安全删除入口可见；本次记录为当前客户端人工验收结果，不替代真实安装包和现场数据全量验收。
...
- 普通 WPS 在线表格同步升级为本地轨旁 AP XLSX 的版式镜像：按本地工作簿顺序重排业务 Sheet，恢复合并、列宽、行高、字体、填充、数字格式、对齐、换行和边框，并将 `_NetConsole*` 系统 Sheet 移到末尾隐藏；`AP上线情况概览` 只格式化本次顶部追加块，非关键 WPS 格式 API 不支持时返回 `SUCCESS_WITH_WARNINGS`，不把已成功写入的核心数据误报为失败。

- 修复 AC 在线 AP 只有 AC 侧 LLDP 上联交换机身份时必须手工执行 ZTE 交换机“详情更新”才能进入轨旁业务的问题：新增同局点唯一交换机身份索引，按 UUID、Chassis/MAC、管理地址、规范化 system name、设备别名依次解析，并通过交换机正式站点与逐站规划生成只读运行态投影；多候选保持冲突，不做模糊匹配或首条回退。
- 轨旁业务页、上线概览与 XLSX 待关联明细统一返回交换机未匹配、交换机身份冲突、交换机站点资料不完整、AP 规划缺失、规划站点缺失/无效等互斥原因码和结构化证据；移除把所有未完成关联都描述为“未关联 AP 规划”的笼统提示。
- 上线概览缓存 revision 纳入 `devices/device_facts/device_interfaces` 交换机身份变化；业务页继续复用来源指纹 R1/R2 查询期原子重建，设备身份、AC LLDP、交换机 LLDP、规划或站点事实变化后自动重算，无需新增刷新按钮、持久快照表或详情更新专用触发逻辑。
...
- 统一 AC/FIT-AP 与轨旁 AP 业务的双侧接收光判定：AP Rx 或交换机 Rx 任一最新有效值严格低于 `-13.90 dBm` 时即标记“光衰大/严重告警”，等于边界值仍为正常，Tx 不使用该接收光门限；后端陈旧 `normal` 不能覆盖数值越界，一侧缺失时综合状态为数据不完整而非正常。`WA6522` 统一为“不适用”，不执行双侧阈值，也不进入光衰异常数量、筛选或导出。
- 2026-08-06 Electron 真实局点人工验收通过：`WA6528X-E` 样本的 AP Rx 为 `-7.72 dBm`、交换机 Rx 为 `-19.10 dBm` 时，交换机侧与综合状态均正确显示“光衰大/严重告警”；本次验收只确认双侧光衰展示与聚合口径，不代表 AC/FIT-AP 全量现场工作流完成。
...
- 2026-08-07 三局点真实 MESH 原始日志、局点数据库和基础资料人工回归通过：宁波 12 号线 `1581/1581`、宁波 6 号线 `51/51` 均按设备现场证据完成匹配；绍兴 1 号线按可匹配范围验收，`eligible=290` 且 `290/290` 成功关联，另有 `164` 条基础资料缺失或无效导致的正常 `unresolved`，异常漏匹配数为 `0`。绍兴不要求所有 Peer 全量匹配；无充分证据的记录保持 `unresolved` 或 `ambiguous`，不得按名称、站点、MAC 前缀或顺序强行关联。
