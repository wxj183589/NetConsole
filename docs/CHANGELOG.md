...
- 设备新增建设阶段、投运状态、状态原因、更新时间和更新人字段；既有设备通过幂等迁移默认保持“已投运”，建设阶段默认为“未指定”，不会改变设备稳定身份或删除历史记录。
- 设备管理支持按建设阶段和投运状态筛选、编辑并展示状态；设备详情、CSV 导入导出和公共导出字段同步携带这些信息，配置采集和设备操作继续复用同一设备状态事实。
- “未接入、调试中、暂停、退役”等非正常投运状态会在设备详情和相关业务入口中明确呈现，后台无人值守与深采调度可据此排除不应参与运行任务的设备。
...
- 旧局点 `devices.db` 的设备地址与建设/投运状态迁移改为可恢复的幂等流程：先创建并校验 SQLite 备份，再在单事务中回填和建立索引；历史地址非法或重复时保留原库并返回局点、设备和迁移阶段诊断，不再生成空替代库。冻结 Backend smoke 同时验证迁移后列表 HTTP 200、重启不重复备份和结构化错误映射。
