...
- 修复 Windows 空目录被安装器误判为“非空普通目录”并要求创建嵌套 `NetConsoleData` 的问题：NSIS 改为在任何创建/写入前逐项枚举并分类目录，分类通过后才执行能力探测，探测后不再按普通目录非空规则复检；空目录直接使用用户所选路径，非空未识别目录只拒绝，不创建、回收、删除、改名或修改用户目录。
...
- 局点导出明确拆分为“完整迁移包”和“脱敏分享包”。完整迁移包要求至少 8 个字符的迁移密码，使用 Scrypt 派生密钥并以 AES-256-GCM 认证加密完整局点载荷；外层包无法搜索到设备密码、SNMP community 或隧道凭据，错误密码、密文或 manifest 被修改时不发布任何数据。
- 脱敏分享包、现场采集包和采集回传包继续清除秘密并设置 `needs_reentry`。是否保留凭据只由包类型决定，不再因跨电脑导入而统一清除；旧 v1/v2 无凭据包保持兼容且不会伪造已被删除的密码。
- 完整包导入新设备时恢复包内凭据；替换已有局点时提供“保留本地凭据/使用包内凭据”策略并默认保留按 `device_uuid` 匹配的本地值。替换发布、Registry 更新或完整性校验失败时恢复原局点，明文临时载荷始终位于隔离数据根 `runtime/temp` 并清理。
- 迁移密码使用 `SecretStr` 接收，只经一次性 Worker stdin 敏感启动帧传递，不写入 Job 参数、`tasks.db`、日志、结果或包 manifest。
- 修复设备编辑的密码保存语义：留空保留原密码，输入新密码后写入并复核，只有显式“清除已保存值”才删除；API 不回传旧明文，旧版误设 `needs_reentry` 但真实完整凭据仍存在时自动修正，真实秘密已丢失时继续要求重录。
...
- v1.4.2 的普通局点包新增非秘密凭据重录状态：导出清空密码、SNMP community 和隧道密码，导入后设备明确显示 `needs_reentry`，连接任务在创建前阻断；v1.4.3 已将该行为限定为脱敏/现场/回传和旧无凭据包，并新增认证加密的完整迁移包。本机数据库凭据存储格式仍保持不变。
