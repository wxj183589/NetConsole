...
- 轨道交通按 Qt 事实源拆分车内点表、轨旁 AP 规划与业务、在线列车 CT/TC、连续采集、Online MR 实时/分析、强停恢复、离线 MESH 导入分析与报告，不再以只读聚合页代替业务闭环。
...
- Vue：58 个测试文件、174 项测试通过，TypeScript 检查和生产构建通过；Electron：10 个测试文件、70 项测试通过，typecheck、main/preload 构建与三类启动冒烟通过。
- 改动范围 Ruff 和文档链接检查通过；全仓 Ruff 的 125 项既有问题未在本次发布中扩大修复范围。
