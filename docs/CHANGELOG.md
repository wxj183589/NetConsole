...
- 统一 AC 与轨旁 AP 的 FIT-AP 运行态口径：`R/M`、`R/B` 显示在线，其余明确 AC 状态显示离线，只有状态字段全部缺失时保留未知；轨旁业务两侧收光功率严格低于 `-13.90 dBm` 时标记“功率异常”，等于边界值仍为正常。
...
- 轨旁 AP 业务新增独立接收光功率维护线：交换机侧和 AP 侧低于 `-13 dBm` 统一标记“功率异常”，并在查询、异常筛选、统计和导出中重算旧快照；设备详情与 ZTE 厂商原生 RX/TX 门限保持不变，更严重的既有状态继续优先。
