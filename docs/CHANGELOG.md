...
- 基础资料中的轨旁 AP 规划移除规划方式、VLAN 组、边界、拆分合并和全局阻断问题，改为可直接编辑、粘贴、新增和删除的一站一行规划表；同 VLAN 跨站合法，IP、掩码和网关只作参考。
...
- “下载模板”生成仅含 `AP规划` 8 列和隐藏 schema v2 元数据的 `轨旁AP逐站规划模板.xlsx`；“导出当前”读取已保存规划，并增加只读 `AP上线情况概览` 工作表，百分比为数值格式、合计行按 719/945 等总量计算。导入只读取 `AP规划`，旧工作表名、“AP数量”表头和旧 22 列 VLAN 分组模板继续兼容；导出复用共享用户目标协调器，取消不创建任务，保存失败保留 Artifact。
