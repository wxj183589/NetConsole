...
- 新增 AP 上线统计子页，按实际有效轨旁 AP 与现有 FIT-AP 状态计算站点及线路加权上线率；规划数量与实际数量允许不同，暂停/退役 AP 排除，未分配 AP 单独警告且不计入线路合计。
- 新模板和当前导出收敛为 8 个逐站业务列，旧 22 列 VLAN 分组模板自动转换并提示；导出继续复用共享用户目标协调器，取消不创建任务，保存失败保留 Artifact。
