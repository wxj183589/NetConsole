...
普通在线表格的数据写入链路已由杭州地铁 10 号线真实 9 Sheet 同步确认，证据等级为 `USER_FIELD_CONFIRMED`。该 `2.2.0-standard` / `trackside-ap-standard-2.2.0` 状态固定标记为 `WPS_STANDARD_DATA_SYNC_LAST_KNOWN_GOOD`。Phase A 的 Sheet 排序、统一 registry、上线历史块和六个标签色也已由用户现场确认。`2.4.1-standard` 的独立列宽探针已确认 A-D 列写后读回和视觉递增，不能重复要求用户验证能力探针。`2.5.0-standard` 增加了正式 143 列的 XLSX、DTO、payload、`ColumnWidth` 和 `Width(points)` 全链路报告。当前候选 `2.8.4-standard` / `trackside-ap-standard-2.8.4` 保留 `2.8.3` 的真实业务列 AutoFit、本地宽度下限、按字段布局 Clamp、FULL_REPLACE 受管区域清理和百分比显示读回，并将 WPS 冻结契约收敛为 `NONE / FIRST_ROW_ONLY`，增加窗格彻底复位、固定 `A2` 选区以及切出切回二次读回。本地 9 Sheet 模拟与杭州 10 号线源 Workbook 审计完成后，真实 WPS 格式报告仍必须标记为 `UNVERIFIED`，发布前不得写成现场已验收。

## Phase A：Sheet 顺序（现场已验证）

普通在线表格复用轨旁 AP 本地 XLSX 的同一次冻结快照和同一个 workbook builder，不维护第二套样式。`TRACKSIDE_AP_BUSINESS_SHEET_DEFINITIONS` 是业务 Sheet 身份、名称、顺序、同步模式、标签色和 WPS `freeze_mode` 的唯一真源；`WorkbookDTO.sheet_order` 无条件按 `openpyxl workbook.worksheets` 的实际枚举顺序从零递增，且排除 `_NetConsole*` 系统 Sheet。固定业务顺序为：`AP上线情况概览`、`轨旁AP业务`、`当前异常光衰`、`AP光衰处理记录`、`AP离线情况`、`AP离线台账`、`新增上线AP概览`、`待关联在线AP`、`交换机光模块统计`。`AP上线情况概览` 与 `新增上线AP概览` 分别使用 `ap_online_history_overview` 和 `newly_online_ap_overview`，不得按中文相似名称混用。
...
冻结窗格在所有数据、格式、AutoFit、筛选、Sheet 排序、系统 Sheet 管理和标签色操作完成后统一执行最终化。WPS DTO 不再透传本地 XLSX 的 `freeze_panes` 地址，只发送 `NONE / FIRST_ROW_ONLY`：`ap_online_history_overview` 固定为 `NONE`，其余八个业务 Sheet 固定为 `FIRST_ROW_ONLY`；旧 `A12 / D20` 等地址即使出现在兼容载荷中也不能控制远端窗格。每页先 Activate 并确认 `ActiveSheet`，关闭 `FreezePanes`，将 `SplitRow / SplitColumn` 归零；运行时存在 `SplitHorizontal / SplitVertical / ScrollRow / ScrollColumn` 时同步归零分割并把视口恢复到左上角。`FIRST_ROW_ONLY` 只选择固定 `A2`、核对 `ActiveCell=2/1`，再设置 `FreezePanes=true`，严禁写入非零 `SplitRow` 或混用两套模型。

每个 Sheet 在设置后立即读回一次，再激活下一个业务 Sheet、切回当前 Sheet 后第二次读回。两次都必须符合目标行数与零冻结列，且可用的 `SplitVertical` 必须为零，才计入 `freeze_panes.verified_count`；报告保留 `before / after_reset / after_select / immediate / reactivated` 的活动 Sheet、活动单元格、Split、Scroll 和可选属性诊断。选择失败、复位失败、立即读回不一致或重新激活后漂移分别记录明确错误码，并返回 `SUCCESS_WITH_WARNINGS`，不得用当场一次读回掩盖真实 UI 错误。Freeze 最终化之后，正式同步只允许写绑定元数据和构造 JSON 结果，不再执行 `Select / Activate / AutoFit / Filter / Move` 或其他 Workbook UI 操作。
...
连接探针只读取 `Context.argv` 与既有 `_NetConsoleSyncMeta`，不创建 Sheet、字段或记录，不清空、不删除、不初始化绑定。读取能力可用时返回 `UNBOUND`、`BOUND`、`LEGACY_BINDING_ID_MISMATCH` 或 `MISMATCH`，并附带本地/远端 Binding ID 和逐项身份匹配结果；智能表格运行时尚未提供 `Application.Sheet.GetSheets / Record.GetRecords` 时，连接仍可判定可达，但绑定状态只返回 `UNKNOWN` 并附带 `binding_readback` warning，必须在部署正式脚本后通过专用运行时探针继续确认。只读探针明确拒绝 `migrate_legacy_binding`；必须先在同一文档共享脚本中部署正式同步脚本，才能执行受控迁移。仓库没有可替代 WPS 编辑器的 AirScript 运行时；四个脚本都必须以顶层 `return main();` 返回 JSON。仅调用 `main();` 或使用 `console.log(main())` 会使 webhook 的 `data.result` 缺少协议结果，即使 WPS 编辑器显示“执行完毕”，也不能视为连接验证通过。
...
智能表格脚本不使用未验证的 `book.Tables`、`DataTables`、`EnsureFields`、`DeleteWhere` 或 `AddRecords`。`2.2.0-smart` 的专用运行时探针只操作 `_NetConsoleSmartProbe`，实际调用并读回 `Application.Sheet.GetSheets/CreateSheet`、`sheet.Field.GetFields/CreateFields`、`sheet.Record.GetRecords/CreateRecords/UpdateRecords/DeleteRecords` 和 `sheet.Move({Before, After})`；`GetRecords` 使用 `Offset` 分页。探针用 20 条测试记录验证当前部署的安全批量值；正式同步只采用该已验证值并继续分批执行，不把它声明为 WPS 官方上限。`View.GetViews` 是可选能力，失败只记录 warning。核心能力全部 PASS 才返回 `runtime_capability=VERIFIED`，否则返回 `WPS_SMART_RUNTIME_PROBE_UNVERIFIED` 并保持正式同步 fail-closed。探针不报告未经验证的隐藏字段支持。

Python 从同一次冻结的 `WorkbookDTO` 生成独立 `SmartWorkbookDTO -> SmartSheetDTO -> SmartFieldDTO / SmartRecordDTO`，智能目标 payload 不再携带普通表格的二维单元格和格式镜像。普通文本、MAC、接口和状态使用 `MultiLineText`，纯数值列使用 `Number`，上线率等比率列使用 `Percentage` 且 `0.81` 原样发送；日期和更新时间在未完成原生 Date 类型探针前保守使用 `MultiLineText`。每条记录包含稳定 `_NC_ROW_KEY`、目标批次 `_NC_BATCH_ID` 和快照 `_NC_REVISION`，这些字段当前保持可见，不虚构隐藏字段能力。

`AP上线情况概览` 在智能表格中使用 `APPEND_SNAPSHOT` 记录语义：每个站点和合计各一条记录，包含日期、更新时间、同步批次、站点、规划总数、上线、未上线、上线率和备注；不复制普通表格的日期块、重复表头或空行。同一 `target_batch_id` 已完整存在时直接按行键读回验证，部分批次只重建该批，不影响旧历史。其余 8 个业务 Data Sheet 使用安全 `FULL_REPLACE`：先分页读取旧记录，再分批创建本批新记录并按数量与 `_NC_ROW_KEY` 读回，验证成功后才分批删除旧批记录；创建或读回失败时保留旧记录。用户额外 Data Sheet 不枚举为受管目标，也不会删除、重命名或清空。

Schema 校验只新增缺失字段；同名字段类型冲突返回失败，未知用户字段保留。九个业务 Data Sheet 使用 Sheet ID 和 `Move({After: id})` 按共享 registry 排序，再次 `GetSheets()` 读回校验。智能排序探针只使用 `_NetConsoleSmartProbeA / B`。智能 `_NetConsoleSyncMeta` 是 Key/Value 系统 Data Sheet，保存独立于普通表格目标的文档、Binding、局点、业务、目标代码和目标类型；首次写入仍必须显式确认初始化，身份不一致时在业务记录写入前 fail-closed。正式同步继续使用异步 `/task + task_id` 轮询。任务详情对智能目标只展示 Data Sheet、字段、记录创建/删除/读回、历史追加、Sheet 顺序和 Binding，不显示普通表格格式项。
...
4. 通过“复制正式同步脚本”手工替换同一个 WPS 文档共享脚本内容并保存、发布；普通表格身份必须是 `2.8.4-standard` / `trackside-ap-standard-2.8.4`，智能表格身份必须是 `2.2.0-smart` / `trackside-ap-smart-2.2.0`，再分别重新测试。NetConsole 不能远程替用户编辑 WPS 脚本，不要把普通表格和智能表格 webhook 交叉使用。
...
7. 点击“测试 Sheet 排序”，普通表格确认 `_NetConsoleRuntimeProbe / _NetConsoleSyncTest` 的位置参数读回；智能表格确认 `_NetConsoleSmartProbeA / B` 的 ID 版 Before/After 读回。两者都必须返回 `sheet_order_verified=true`、`sheet_move_before_verified=true`、`sheet_move_after_verified=true`。该探针不并入“重新验证当前部署”。
...
12. 远端状态进入 `finished` 后，在任务详情核对列宽的本地值、AutoFit 值、最终请求值、Clamp、远端读回、验证、告警和失败数量，以及 RowHeight、Font、Fill、NumberFormat、Alignment、Wrap、Merge、Border、FreezePane、Filter 和业务样本的写后读回结果。杭州 10 号线当前源应报告 143 个真实 AutoFit 请求；Freeze 必须报告 9/9，且每项的 `immediate` 与 `reactivated` 同时通过：概览为 `0/0`，其余八页为 `1/0`。重点查看 `轨旁AP业务` 的 A/B/C/G/H/P 代表列，以及 `AP上线情况概览` 的 `0.81 / 0.0% / 81.0%` 样本。不能只依据 API 未抛异常判断格式成功。
