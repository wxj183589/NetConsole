...
`module.ground_unattended` 是轨道交通下的独立正式页面，依赖任务中心、轨道交通基础资料和列车在线能力。页面维护自己的局点配置、运行、全车长 Ping、每日深度覆盖和归档状态；底层复用 AC Mesh-Link、Online MR Session/最终化和 fping，不建立第二套人工采集页面。该页面代码与自动测试已接入生产基线；真实 MR、长时运行和托盘隐藏验收状态仍为 `PENDING_REAL_DEVICE`，不能由 HZ10 AC/FIT-AP 只读证据替代。
