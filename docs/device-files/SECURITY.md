...
- 自动启用必须由 Application Service 经 `DeviceOperationService` 提交到现有 Task Center，并由 H3C Comware major-family 或已证实 Release override 的 `device.sftp.enable` Profile 执行；未知厂商、角色、平台和 major 不得猜测或回退，未登记 Patch/Release 不单独否定已确认的 V7 family。
